...
\begin{theorem}
	...
	Then, $H_\alpha(\w) \leq H_\alpha(\w')$, where $\alpha$ is such that $\hspace{-1pt}H_\alpha(\x)\hspace{-2pt}$ is well-defined for all quasi-distributions $\x$.
\end{theorem}
\begin{proof}
Given a fixed $\alpha$ for which $H_\alpha(\x)$ is well-defined for all quasi-distributions $\x$, the function $H_\alpha$ is real-valued, smooth on the interior of $\mathcal{Q}^\downarrow$ and symmetric on permutation components of $\x$.

Consider the partial derivatives
\begin{equation}
	\frac{\partial H_\alpha}{\partial w_i} = \frac{\alpha w_i^{\alpha-1}}{(1-\alpha)\sum_k{w_k^\alpha}} 
\end{equation}
Whenever $w_i \geq w_j$, we have that 
\begin{equation}
	\frac{w_i^{\alpha-1}}{1-\alpha} \leq \frac{w_j^{\alpha-1}}{1-\alpha},
\end{equation}
\nick{wrong} independently of whether $\alpha < 1$ or $\alpha > 1$, which leads to
\begin{equation}
...
We can now derive the following entropic bounds on magic state transformations. \nick{to be extended (1) from unital processes to general relative majorization, (2) by dropping the condition $\alpha > 1$}
