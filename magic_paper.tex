...
\begin{equation}\label{eq:lcsu}
	\lc{\rho_{\rm{S}}(\epsilon)^{\otimes n}} {\frac{1}{d}\id}(x).
\end{equation}
...
\textbf{Notational convention:} For any non-negative reals $a,b$ with $b\geq a$, we will denote the set of integers $n$ such that $a\leq n \leq b$ as $[a,b]$ for convenience.\\
...
\begin{align}	
	...
	&m\in[0,n]\ \text{and m even}, \label{eq:fp} \\
	\Phi_-(m; n, p) &\coloneqq \sum\limits_{j=1}^{(m-1)/2} \binom{n}{2j+1} p^{2j+1} (1-p)^{n-(2j+1)}, \nonumber\\ 
	&m\in[0,n]\ \text{and m odd}. \label{eq:fn2}
\end{align}
Index $m$ only takes even (odd) values when labelling $\Phi_+$ ($\Phi_-$) and therefore we can use $\Phi_{\pm}$ to compactify statements that hold for both sums.
