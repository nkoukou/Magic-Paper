...
\begin{equation}
\D = \{ \w \in \mathbb{R}^N: \sum_{i=1}^N w_i = 1\}.
\end{equation}
...
\begin{lemma} 
Let $f$ be a real-valued continuous function defined on $\D^\downarrow$ that is continuously differentiable on the interior of $\D^\downarrow$. Then, $f$ is Schur-convex on $\D^\downarrow$ if and only if $\partial_k f(\w)$ is decreasing in $k$ for all $\w \in \D^\downarrow$, namely
\begin{equation}
...
\end{theorem}
\begin{proof}
Given a fixed $\alpha$ for which $H_\alpha(\x)$ is well-defined for all quasi-distributions $\x$, the function $H_\alpha$ is real-valued, smooth on the interior of $\D^\downarrow$ and symmetric on permutation components of $\x$.
...
\end{equation}
independently of whether $\alpha < 1$ or $\alpha > 1$, which leads to
\begin{equation}
