...
\subsubsection*{Strange state distillation in the unital fragment}\label{sec:lcsu}

In this section we study the MSD process,
\begin{equation}\label{eq:sudist}
	\noisysn \xrightarrow{\E \in \O_{\id/3}} \noisys(\epsilon')\too{\tt} \otimes \left( \frac{1}{3}\id \right)\too{\mm},\ \nn = \tt + \mm,
\end{equation}
where we have uniformised the number of qutrits on both sides of the process.
...
\end{equation}
We call Lorenz curves of this type \emph{standard Lorenz curves}. \nick{expand (non-uniformity)}
...
Here we focus on the part of the process described by the LHS of the Lorenz curves, in low noise regimes ($0 \leq \epsilon < 3/7$) with even numbers of initial and target copies.
In this case, the elbow expressions of the standard Lorenz curve in~\cref{eq:lcsu} are given by
\begin{align}
	x_i &= \Phi_+\left(2\ii;\NN,\frac{8}{9}\right), \\
	L_i &= \left( \frac{5}{3} - \frac{8}{9}\epsilon\ \right)^\NN \Phi_+\left(2\ii;\NN,\frac{12-4\epsilon}{15-8\epsilon}\right),
\end{align}
for $i=0,\dots,\frac{\NN}{2}$, where
\begin{equation}
	\Phi_+(2\mmf; \nnf, p) \coloneqq \sum\limits_{\dummy=0}^{\mmf} \binom{\nnf}{2\dummy} p^{2\dummy} (1-p)^{\nnf-2\dummy}.
\end{equation}
Conventionally, we set $(x_{-1}, L_{-1}) = (0,0)$.
...
\end{equation}
with $\ii=0,\dots,\frac{\nn}{2},\ \jj(\ii) = 1,\dots,8^{2\ii}\binom{\nn}{2\ii}$ and $\kk = 1$.
For the target state $\noisys(\epsilon')\too{\tt} \otimes \left( \frac{1}{3}\id \right)\too{\mm}$,
\begin{equation}
	p_{\ii'\jj'\kk'} = \frac{\kk' + (\jj'-1)9^{\mm}}{9^{\mm} 8^{2\ii'}\binom{\tt}{2\ii'}}
\end{equation}
with $\ii'=0,\dots,\frac{\tt}{2},\ \jj'(\ii') = 1,\dots,8^{2\ii'}\binom{\tt}{2\ii'}$ and $\kk' = 1,\dots,9^{\mm}$.

Pointwise Lorenz curve comparison requires $x_{\ii\jj\kk} = x_{\ii'\jj'\kk'}$, so we need ot match up the indices appropriately in order to compare $L_{\ii\jj\kk}$ and $L_{\ii'\jj'\kk'}$.
In~\cref{app:lc_compare}, we show that the requirement on the indices is finding a tuple $(\ii, \jj(\ii))$, such that for a given $\ii' = 0,\dots,\frac{\tt}{2}$,
\begin{equation}
	\jj + \sum_{\dummy=0}^{\ii-1} 8^{2\dummy}\binom{\nn}{2\dummy} = \sum_{\dummy=0}^{\ii'} 8^{2\dummy}\binom{\tt}{2\dummy}.
\end{equation}
