...
\begin{align}
    ...
    &\left\{ \left( 8^{n-2k}\binom{n}{2k}, \left( \frac{1}{6} - \frac{1}{18}\epsilon \right)^{2k}\left( -\frac{1}{3} + \frac{4}{9}\epsilon \right)^{n-2k} \right) \right\}_{k=0,\dots,n/2}. \nonumber
\end{align}
...
\begin{equation}
    \rho(m, \epsilon, s) = \rho_{\rm{S}}(\epsilon)^{\otimes m} \otimes \left( \frac{1}{3}\id \right)^{\otimes s}.
\end{equation}
...
\begin{align}
    ...
    &\text{for } k=0,\dots,\frac{n}{2},\ \alpha = 1,\dots,m_k \text{ and } \beta = 1,\dots,9^s. \nonumber
\end{align}

We can unify the indices, by introducing a single index
\begin{equation}
    i(k,\alpha,\beta) \coloneqq \beta + \left[ (\alpha-1) + \sum_{j=0}^{k-1} 8^{m-2j}\binom{m}{2j} \right]9^s,
\end{equation}
so that $i=1,2,\dots, \frac{1}{2}(7^{m+s} + 9^{m+s})$.
The $\frac{m}{2}$ elbows correspond to $\beta=0,\ \alpha=1$.
...
\begin{equation}\label{eq:su_conversion}
    \noisysn \xrightarrow{\mathcal{O}_{\id/d}} \rho_{\rm{S}}(m, \epsilon', s).
\end{equation}
...
\begin{align}
    i(k, \alpha) &= (\alpha-1) + \sum_{j=0}^{k-1} 8^{n-2j}\binom{n}{2j}, \\
    i'(k',\alpha',\beta') &= \beta' + \left[ (\alpha'-1) + \sum_{j'=0}^{k'-1} 8^{m-2j'}\binom{m}{2j'} \right]9^s,
\end{align}

The question is: \emph{Given a triplet $(k', \alpha', \beta')$, what is the tuple $(k, \alpha)$ such that $i(k, \alpha) = i'(k',\alpha',\beta')$?}
...
Returning to~\cref{eq:su_conversion}, the $k$ independent necessary constraints are
\begin{align}
\lc{\noisysn}{\frac{1}{d}\id}(x_{1,0,0}) &\geq \lc{\rho_{\rm{S}}(\epsilon_{\tau})^{\otimes k}}{\frac{1}{d}\id}(x_{1,0,0}), \nonumber\\
\lc{\noisysn}{\frac{1}{d}\id}(x_{2,0,0}) &\geq \lc{\rho_{\rm{S}}(\epsilon_{\tau})^{\otimes k}}{\frac{1}{d}\id}(x_{2,0,0}), \nonumber\\
&\vdotswithin{=} \nonumber\\
\lc{\noisysn}{\frac{1}{d}\id}(x_{m,0,0}) &\geq \lc{\rho_{\rm{S}}(\epsilon_{\tau})^{\otimes k}}{\frac{1}{d}\id}(x_{m,0,0}), \nonumber
\end{align}
where $x_{1,0,0}, x_{2,0,0}, \dots, x_{m,0,0} \in T \subset X$ are the locations of the elbows of $\lc{\rho_{\rm{S}}(\epsilon_{\tau})^{\otimes k}}{\frac{1}{d}\id}$ (excluding $0$ and $1$).
