...
\begin{equation}
	\alpha_k \coloneqq \sum_{\mathbb{Z}_d} \braket{\phi_k}{-r}\braket{r}{\phi_k}.
\end{equation}
Coefficients $\alpha_k$ are not necessarily positive, which is the case when the Hamiltonian $H$ does not admit a stabilizer decomposition. 
...
\end{equation}
where for simplicity we write $\rho_n \coloneqq \rho_S(\epsilon)^{\otimes n}$, $\rho'_m\coloneqq \rho_S(\epsilon')^{\otimes m}$ and $\tau_n\coloneqq\tau^{\otimes m}$ and $\sigma_m\coloneqq\sigma^{\otimes m}$. We then have that
\begin{equation}
