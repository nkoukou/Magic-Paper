...
In particular, any $\sigma$--fragment, with $\sigma$ not full-rank, can be approximated by the $\sigma'$--fragment defined as a mixture of $\sigma$ with depolarising noise, $\sigma' = (1-\epsilon)\sigma + \epsilon\frac{1}{d}\id$, for some infinitesimal $\epsilon > 0$, so that $\sigma'$ is full-rank and arbitrarily close to $\sigma$.
...
\end{equation}
in $\O_\sigma$, where $\ket{\rm{S}}$ is coined in~\cref{fig:strange} and we induce uniform depolarising noise.
...
Finally, we assume a range of initial noise levels, $\epsilon \leq 3/7$  \nick{higher than all threshold values CITE}, so that the largest Wigner component of the 1-copy noisy magic state is negative.
...
We allow for depolarising noise, so that the system state is a convex sum of the pure Strange state and the maximally mixed state,
\begin{equation}
...
\end{equation}
and compare with the numerically optimal error bounds provided by majorization as shown in~\cref{fig:prakash_compare_errors}
\begin{figure}[h]
    \centering
...
\end{equation}
where $\Lambda^+(\rho)$ is the generalised robustness of state $\rho$, calculable via a costly SDP and $F(\psi)$ denotes the stabilizer fidelity of state $\ket{\psi}$.
...
\end{equation}
Their bound simply states that any deterministic single-copy distillation process $\rho^{\otimes n} \longrightarrow \tau$ requires an initial number of copies $n \geq \theta_{\rm{max}}(\rho) / \theta_{\rm{max}}(\tau)$.
...
\end{equation}
Therefore, given process parameters $n, n', \epsilon, \epsilon'$, we can scan through the stabilizer states to find a range of $\beta$ for which inequality~\cref{eq:rate_cost} is valid.
