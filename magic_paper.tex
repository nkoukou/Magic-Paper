...
>>>>>>> 257f490dd2e77f4ec690559482d8aeb6d66a7bc3
\end{equation}
...
<<<<<<< HEAD
\begin{proof} If we have $H_\alpha (W_\rho(\x)) <0 $ then this implies that $\log \sum_{\x} W_\rho (\x)^\alpha > 1$, however it is known that $H_\alpha$ is always non-negative on probability distributions and therefore $W_\rho(\x)$ must be a strict quasi-distribution with negativity.
=======
...
\end{align}
where $\epsilon = a/b - 1 >0$. By choosing $a$ and $b$ appropriately we can make $\epsilon$ arbitrarily close to $0$ and therefore there exists a sequence $\epsilon_n=a_n/b_n$ such that
\begin{equation}
