...
We can reduce the number of independent constraints stemming from this inequality down to the number of elbows of the target state.
\begin{theorem}\label{thm:elbows}
	...
	Let $r$ be the number of elbows of $\lc{\tau}{\sigma}(x)$ and $E \coloneqq \{x_1, \dots, x_\kk\}$ be their locations.
	
	Then, $\lc{\rho}{\sigma}(x) \geq \lc{\tau}{\sigma}(x)$ for all $x \in [0,1]$ iff $\lc{\rho}{\sigma}(x_j) \geq \lc{\tau}{\sigma}(x_j)$ for all $j =1,\dots,\kk$.
\end{theorem}
\begin{proof}	
	$\lc{\rho}{\sigma}(x) \geq \lc{\tau}{\sigma}(x)$ for all $x \in [0,1]$ trivially implies $\lc{\rho}{\sigma}(x_j) \geq \lc{\tau}{\sigma}(x_j)$ for all $j =1,\dots,\kk$.
	
	Conversely, assume that $\lc{\rho}{\sigma}(x_j) \geq \lc{\tau}{\sigma}(x_j)$ for all $j =1,\dots,r$.
	First, let $x_0 = 0$ and $x_{\kk+1} = 1$, so that $\lc{\rho}{\sigma}(x_0) = \lc{\tau}{\sigma}(x_0) = 0$ and $\lc{\rho}{\sigma}(x_{\kk+1}) = \lc{\tau}{\sigma}(x_{\kk+1}) = 1$.
	Hence, we can extend the set of elbows $E$ to $E' = E \cup \{x_0, x_{\kk+1}\}$.
	
	Pick two consecutive locations $x_i, x_{i+1}$ in $E'$ and consider the line segment $\ell_\tau(x)$ connecting points $(x_i, \lc{\tau}{\sigma}(x_i))$ and $(x_{i+1}, \lc{\tau}{\sigma}(x_{i+1}))$ as well as the line segment $\ell_\rho(x)$ connecting points $(x_i, \lc{\rho}{\sigma}(x_i))$ and $(x_{i+1}, \lc{\rho}{\sigma}(x_{i+1}))$.
...
\end{figure}
	...
	This argument can be made in all intervals $[x_i, x_{i+1}]$ with $i=0,\dots,\kk$, so the proof is complete.
\end{proof}
This theorem is of practical importance in checking Lorenz curve bounds, and it is used in the discussion of standard Lorenz curves in~\cref{sec:lcsu}.
...
\begin{proof}
	...
	Let $\jj_\star$ be the index of the smallest non-negative component of $\tilde{\bm{w}}^\downarrow$.
	Then, $(\W{\rho})_i < 0$ if and only if $i > \jj_\star$, so the maximum of Lorenz curve $\lc{\rho}{\sigma}(x)$ takes the value 
	\begin{equation}
		\lc{\rho}{\sigma}(x_{\jj_\star}) = \sum_{i=1}^{\jj_\star} (\W{\rho})_i,
	\end{equation}
	and is achieved at
	\begin{equation}\label{eq:maxloc}
		x_{\jj_\star} \coloneqq \sum_{i=1}^{\jj_\star} (\W{\sigma})_i.
	\end{equation}
...
\begin{equation}
		\rho(\epsilon)\too{\nn} \xrightarrow{\E \in \O_\sigma} \rho(\epsilon')\too{\kk},
\end{equation}
where it is converted into a $\kk$-copy, $\epsilon'$-noisy state, such that the number of copies has decreased ($\kk < \nn$), but the state is less noisy ($\epsilon' < \epsilon$).
...
\begin{equation}\label{eq:ethresh}
	\begin{split}
	\epsilon_{\rm{th}}(\nn, \kk, \epsilon', \sigma) \coloneqq &\sup{\{\epsilon:\ \lc{\rho(\epsilon)\too{\nn}}{\sigma}}(x) \geq \\
	&{\lc{\rho(\epsilon')\too{\kk}}{\sigma}(x),\ x\in[0,1]\}}.
	\end{split}
\end{equation}
...
In general, the Wigner components of an $n$-copy state $\rho\too{n}$ can be calculated directly from $\W{\rho}$ by convolution of the distribution with itself,
\begin{equation}
...
\subsection{Magic in the unital fragment}\label{sec:unital}
...
\begin{figure}[b]
    ...
    \caption{\textbf{Noise thresholds in unital fragment.} Mana and Lorenz curve noise thresholds for the Strange state purification process in~\cref{eq:sdist} with $\epsilon' = 0.05$.
...
\nick{We devote a section on Strange state Lorenz curve analysis in the unital fragment}

\null\newpage

\subsection{Strange state interconversions in the unital fragment}\label{sec:lcsu}

In this section we analytically investigate the behaviour of the Lorenz curve of an $n$-copy Strange state in the unital fragment ($\sigma = \id/3$), 
\begin{equation}\label{eq:lcsu}
	\lc{\noisysn}{\frac{1}{d}\id}(x),\ x\in[0,1].
\end{equation}
We recognise Lorenz curve of this type as ``standard candles'' in the context of magic state distillation and call them the \emph{standard Lorenz curves} \nick{expand}.

The aim is to study the MSD process,
\begin{equation}\label{eq:sudist}
	\noisysn \xrightarrow{\E \in \O_{\id/3}} \noisys(\epsilon')\too{\kk},
\end{equation}
in the regime where $0 \leq \epsilon \leq \tfrac{3}{4}$.
For $\tfrac{3}{4} \leq \epsilon < 1$, the noisy Strange state $\noisys(\epsilon)$ contains no Wigner negativities in the unital fragment as seen in~\cref{eq:wsu}.

\subsubsection{Coordinate expressions for standard Lorenz curves}
...
\begin{equation}\label{eq:wu}
	\W{\left(\id/3\right)\too{\NN}} = \left( \overbrace{\frac{1}{9^\NN}, \dots, \frac{1}{9^\NN}}^{9^\NN} \right).
\end{equation}
...
\begin{equation}\label{eq:wsu}
	\W{\noisys(\epsilon)} = \left( \overbrace{\frac{1}{6} - \frac{1}{18}\epsilon, \dots, \frac{1}{6} - \frac{1}{18}\epsilon}^8, \overbrace{-\frac{1}{3} + \frac{4}{9}\epsilon}^1 \right)
\end{equation}
The Wigner distribution of the $\NN$-copy $\epsilon$-noisy Strange state $\noisysN$ in the unital fragment is the $n$-th convolution $\W{\noisysN} = \rm{W}_{\noisys(\epsilon)}\too{N}$.

For example, the Wigner distribution of state $\noisys(0)\too{2}$ is
\begin{equation*}
	\rm{W}_{\noisys(0)\too{2}} = \left( \overbrace{\left( -\frac{1}{3} \right)^2}^1, \overbrace{\left( \frac{1}{6} \right)^2, \dots, \left( \frac{1}{6} \right)^2}^{64}, \overbrace{ -\frac{1}{3} \cdot \frac{1}{6}, \dots, -\frac{1}{3} \cdot \frac{1}{6}}^{16} \right)
\end{equation*}

In~\cref{app:lcsu_technical}, we derive explicit expressions for the coordinates of standard Lorenz curves, which we present here.

\nick{noisy maximum}
Calculating the components of $\ket{S}\bra{S}\too{n}$ allows for analytical calculation of the maximum of the $n$-copy state,
\begin{equation}\label{eq:Skmax}
    \max_x{\lc{\ketbra{S}\too{n}} {\sigma}}(x) = \frac{1}{2}\left (1 + \left(\frac{5}{3}\right)^n \right).
\end{equation}
...
\subsubsection{Standard Lorenz curve comparison}

Consider the Strange state purification process in the unital fragment,
\begin{equation}\label{eq:su_conversion}
    \noisysn \xrightarrow{\mathcal{O}_{\id/d}} \rho_{\rm{S}}(r, \epsilon', s),\ n = r + s.
\end{equation}
In order to get Lorenz curve constraints on this state conversion, we need a method to match up indices between the two curves.
We use non-primed indices for the initial state and primed indices for the target state,
\begin{align}
    i(k, \alpha, \beta=1) &= 1 + (\alpha-1) + \sum_{j=0}^{k-1} 8^{2j}\binom{n}{2j}, \\
    i'(k',\alpha',\beta') &= \beta' + \left[ (\alpha'-1) + \sum_{j'=0}^{k'-1} 8^{2j'}\binom{r}{2j'} \right]9^s,
\end{align}
for $i,i' = 1,2,\dots, \frac{1}{2}(7^{n} + 9^{n})$.

For pointwise comparison of the two curves, we require $x_{i(k, \alpha)} = x_{i'(k', \alpha', \beta')}$, so the question is: \emph{Given a triplet $(k', \alpha', \beta')$, what is the tuple $(k, \alpha)$ such that $i(k, \alpha) = i'(k',\alpha',\beta')$?}

In other words, we choose a fixed $i'(k',\alpha',\beta')$ and we are searching for $i(k, \alpha)$ such that 
\begin{equation}
	i(k, \alpha) = \alpha + \sum_{j=0}^{k-1} 8^{2j}\binom{n}{2j} \overset{!}{=} i'(k',\alpha',\beta').
\end{equation}
...
\begin{equation}
	L_{i(k, \alpha)} \geq L'_{i'(0, 1, 81)}
\end{equation}
where the first multiplicity of the target state is $m_0 = 1$.
The multiplicities of the initial state are $(1, 384, 4096)$
Note that the second elbow of the target state (its maximum) lies on the RHS of the initial state curve's maximum, hence our expressions do not work on the coordinates of the initial state curve at that point \nick{yet}.

The challenge is to find $k, \alpha$ such that $i(k, \alpha) = i'(0,1,81)$.
\nick{By trial and error}, we find that $(k, \alpha) = (1, 80)$.
...
\nick{Returning to~\cref{eq:su_conversion}, and }using~\cref{thm:elbows}, the $r$ independent necessary constraints are
\begin{align}
\lc{\noisysn}{\frac{1}{d}\id}\left(x_{i'(0, m'_{0}, 9^{s})}\right) &\geq \lc{\rho_{\rm{S}}(\epsilon')\too{k}}{\frac{1}{d}\id}\left(x_{i'(0, m'_{0}, 9^{s})}\right), \nonumber\\
\lc{\noisysn}{\frac{1}{d}\id}\left(x_{i'(1, m'_{1}, 9^{s})}\right) &\geq \lc{\rho_{\rm{S}}(\epsilon')\too{k}}{\frac{1}{d}\id}\left(x_{i'(1, m'_{1}, 9^{s})}\right), \nonumber\\
&\vdotswithin{=} \nonumber\\
\lc{\noisysn}{\frac{1}{d}\id}\left(x_{i'(r, m'_{r}, 9^{s})}\right) &\geq \lc{\rho_{\rm{S}}(\epsilon')\too{k}}{\frac{1}{d}\id}\left(x_{i'(r, m'_{r}, 9^{s})}\right), \nonumber
\end{align}
where $x_{i'(0, m'_{0}, 9^{s'})}, x_{i'(1, m'_{1}, 9^{s'})}, \dots, x_{i'(r, m'_{r}, 9^{s'})} \in T$ are the locations of the elbows of $\lc{\rho_{\rm{S}}(\epsilon')\too{r}}{\frac{1}{d}\id}$ (excluding $0$ and $1$).
