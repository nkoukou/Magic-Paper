...
It is the simplest non-trivial case to analyse because the Strange state only has two distinct components $\left\{ -\frac{1}{3}, \frac{1}{6} \right\}$ with multiplicities $1$ and $8$ respectively.
...
\end{equation}
independent of the particular $\sigma$--fragment. \ddd{[We should also give for what value of $x$ this maximum occurs.]} \nick{It will have the form of~\cref{eq:maxloc} and probably cannot be simplified more other than finding $k_\star$ exactly.}\ddd{[Finding $k_\star$ is perfect. We'll always have the complexity of the free state distribution to deal with. For the unital fragment and thermal fragment this will be computable though.]}
...
\begin{figure}[t]
    ...
    \caption{\textbf{Noise thresholds in thermal fragments.} Lorenz curve ratios for the Strange state purifying process in~\cref{eq:sdist} with $\epsilon_\tau = 0.05$.
