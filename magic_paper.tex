...
\begin{theorem}\label{thm:elbows}
	...
	Then, $\lc{\rho}{\sigma}(x) \geq \lc{\tau}{\sigma}(x)$ for all $x \in [0,1]$ iff $\lc{\rho}{\sigma}(x_{\ii}) \geq \lc{\tau}{\sigma}(x_{\ii})$ for all $\ii =1,\dots,n'$.
\end{theorem}
...
In general, the Wigner components of an $\NN$-copy state $\rho^{\otimes \NN}$ can be calculated directly from $\W{\rho}$ by convolution of the distribution with itself,
\begin{equation}
	\W{\rho^{\otimes \NN}} = \rm{W}_{\rho}^{\otimes \NN},
\end{equation}
...
We consider the problem of purifying $n$ copies of a noisy strange state $\noisysn$ into a smaller number of copies $n'$ of a less noisy strange state $\noisys(\epsilon')^{\otimes n'}$, with $\epsilon' \le \epsilon$ and $n' \leq n$. 
...
\begin{equation}\label{eq:sudist}
	\noisys^{\otimes n} \longrightarrow \noisys(n', \epsilon', n - n') \coloneqq  \noisys(\epsilon')^{\otimes n'} \otimes \left( \frac{1}{3}\id \right)^{\otimes (n-n')},
\end{equation}
...
\end{align}
where we use a three-index parametrisation for the coordinates, with $\ii$ the index of the elbow that follows, $\jj = 1,\dots,\mlt_\ii$ and $\kk=1,\dots,9^{2(n-n')}$.
Therefore, for the initial state $\noisys^{\otimes 2n}$ we have
\begin{equation}
	p_{\ii\jj\kk} = \frac{\jj}{8^{2\ii}\binom{2n}{2\ii}},
\end{equation}
with $\ii=0,\dots,n,\ \jj(\ii) = 1,\dots,8^{2\ii}\binom{2n}{2\ii}$ and $\kk = 1$.
For the target state $\noisys(\epsilon')^{\otimes 2n'} \otimes \left( \frac{1}{3}\id \right)^{\otimes 2(n-n')}$,
\begin{equation}
	p'_{\ii'\jj'\kk'} = \frac{\kk' + (\jj'-1)9^{2(n-n')}}{9^{2(n-n')} 8^{2\ii'}\binom{2n'}{2\ii'}}
\end{equation}
with $\ii'=0,\dots,n',\ \jj'(\ii') = 1,\dots,8^{2\ii'}\binom{n'}{2\ii'}$ and $\kk' = 1,\dots,9^{2(n-n')}$.
\fi
...
\nick{This is in general true, will check if in our case it makes a difference}
...
\begin{equation}\label{eq:msd_thermal}
		\noisys(\epsilon)^{\otimes n} \xrightarrow{\E \in \O_{\gamma_\beta}} \noisys(\epsilon')^{\otimes n'} \otimes \gamma_\beta^{\otimes (n-n')},
\end{equation}
...
\begin{equation}\label{eq:first_elbow_constraint}
	\lc{\noisys(\epsilon)^{\otimes n}}{\gamma_\beta}(x_\star) \geq \lc{\noisys(\epsilon')^{\otimes n'}}{\gamma_\beta}(x_\star).
\end{equation}
