...
I include in~\cref{fig:test} the mana and max--thauma bounds for the purification of (a) Norrell states and (b) $T$ states.
...
\end{equation}
where, of course, $d=3$. So in $t_0^n$ we immediately get a term $\Z^{-n} = e^{-nF}$, which is nice and general. Using the expansion of the thermal state into its energy eigenstates we obtain
\begin{align}
t_0^n &= \left ( \frac{1}{d\Z}\tr [ A_{0,0} e^{-\beta H}] \right )^n \nonumber \\
&= \frac{e^{-nF}}{3^n} ( \tr [ A_{0,0} \sum_k e^{-\beta E_k} |\varphi_k\>\<\varphi_k|])^n\nonumber \\
&= \frac{e^{-nF}}{3^n} ( \sum_k \alpha_k e^{-\beta E_k})^n,
\end{align}
where we define
\begin{equation}
