...
\end{itemize}
The intermediate terms specify how the energy eigenbasis of the system $\{|E_k\>\}$ relates to the computational stabilizer basis $\{|k\>\}$.  In particular, the term $\phi$ quantifies to what degree a sharp energy value can be ascribed to the negativity in the Wigner representation. Its form is similar to $F$, and so $\phi$ can informally be viewed as a `magic free energy' term. 
...
\begin{equation}
D_\infty (\rho , \tau) \ge D_\infty (\E(\rho), \E(\tau))
\end{equation}
for any free operation $\E$. \ddd{[True? Can we say a bit more?]}. Note though that $D_\infty (\rho,\tau) = 0$ can also occur for $\rho \ne \tau$. It would be of interest to further explore this more abstract perspective.
