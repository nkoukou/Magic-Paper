...
\begin{equation}
	\sum_{j=0}^{n} c_j = 9^n
\end{equation}
\nick{technically $(9^n+1)$ because 0 is artificially added in the end} points with $x$-coordinates incrementing by $\frac{1}{9^n}$ which we denote as the set 
\begin{equation}
X \coloneqq \left\{ \sum_{i=0}^{k} \frac{i}{9^n} \right\}_{0 < k \leq 9^n}.
\end{equation}
\ddd{[DJ: This sum can be evaluated: $\sum_{i=0}^k = \frac{1}{2}k(k+1)$. Always aim to tidy and simplify! Half of research is presenting the facts simply and clearly.]}

The final state Lorenz curve consists of $9^k (< 9^n)$ points and, \ddd{however since we work in the unital fragment the maximally mixed state$\frac{1}{d} \I$ is a free state and so we are free to add in arbitrary many copies to the final state. This addition does not change the total Lorenz curve of the state, but merely increases the resolution on the $x$ axis. We thus want to compute the Lorenz curve of the state}
\begin{equation}
...
\end{equation}
and multiplicities $c_j^\prime = d^{2(n-k)}c_j$, where $w_j, c_j$ are given in~\cref{tab:lcsu_w} for $j=0,\dots,k$. \ddd{[DJ: You've been too sloppy and rushed here. Give the explicit definition of $w'_j$. You've hit a problem almost certainly because you are not handling this technical bit carefully and explicitly - it is crucial, so really unpack it slowly!]}
The Lorenz curve of the final state remains the same, but now it consists of $9^n$ points with the same $x$-coordinates as the initial state \ddd{[DJ: I don't think this is true. You should step through the computation of the Lorenz curve more carefully and see why]}.
Therefore, we can compare the curves pointwise with $9^n$ inequalities of the form
...
\begin{theorem}\label{thm:elbows}
	...
	Then, $\lc{\rho}{\sigma}(x) \geq \lc{\tau}{\sigma}(x)$ for all $x \in [0,1]$ iff the elbows of $\lc{\tau}{\sigma}(x)$ lie vertically below $\lc{\rho}{\sigma}(x)$.
\end{theorem}
\begin{proof}
	Let $n$ be the number of elbows of $\lc{\tau}{\sigma}(x)$ and $T \coloneqq \{x_0, x_1, \dots, x_n, x_{n+1} \}$ be the set of the elbows' $x$-coordinates including $x_0 = 0$ and $x_{n+1} = 1$.
	
	$\lc{\rho}{\sigma}(x) \geq \lc{\tau}{\sigma}(x)$ for all $x \in [0,1]$ implies $\lc{\rho}{\sigma}(x_k) \geq \lc{\tau}{\sigma}(x_k)$ for all $x_k \in T$ trivially.
	
	Conversely, assume that $\lc{\rho}{\sigma}(x_k) \geq \lc{\tau}{\sigma}(x_k)$ for all $x_k \in T$.
...
\end{figure}
	...
	This argument can be made in all intervals $[x_i, x_{i+1}]$ with $i=0,\dots,n$, so the proof is complete.
\end{proof}
...
\end{align}
where $x_1, x_2, \dots, x_k \in X$ are the locations of the elbows of $\lc{\rho_{\rm{S}}(\epsilon_{\tau})^{\otimes k}}{\frac{1}{d}\id}$.
