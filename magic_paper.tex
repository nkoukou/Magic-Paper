...
% Figs check(fig.0, move to app, consistency, cbars, iffalse, axes)

% Notation consistency (noise vs error, \ln{} -> \ln, \ln vs \log vs \log_2, italics & \emph, Aberg -> {\AA}berg, quasi-dist vs quasi-prob dist)
% UK vs US english
...
\ddd{[Title of Gour2020 paper is incorrect in the bibliography]}
...
\end{theorem}
For magic protocols that generate unital maps the above bound reduces to the simpler form
\begin{equation}
...
\end{equation}
For any $\epsilon$ the upper bound can be numerically minimized over $\alpha$.
\begin{figure}[t!]
    \centering
    \subfigure[][]{%
    \label{fig:Ra}%
