...
\begin{equation}\label{eq:ethresh}
	n_{\rm{th}}(\epsilon, \epsilon_\tau, \sigma) \coloneqq \min{\{n:\ n \in \mathbb{N}, \lc{\rho_{\rm{S}}(\epsilon)^{\otimes n}}{\sigma} \geq \lc{\rho_{\rm{S}}(\epsilon_{\tau})}{\sigma}\}}.
\end{equation}}
...
We now want to bound these points for any index $m$, so as to investigate the asymptotic behaviour of the Lorenz curve.
We define the upper bound
\begin{equation}
	U(m;n,p) \coloneqq 
	\begin{cases}
	e^{-nS\left(\frac{m}{n}||p \right)} \label{eq:ubd},&\ m \leq np, \\
	1,&\ m>np.
	\end{cases}
\end{equation}
\nick{we need to replace 1 with an exponential decay}
where 
\begin{equation}
...
There are two useful lower bounds for $\Phi(m;n,p)$, which we define as
\begin{equation}
	L(m;n,p) \coloneqq \max{\left\{ c(m;n) U(m;n,p), \\ 1 - U(n-m;n,1-p) \right\}} \label{eq:lbd},
\end{equation}
where
\begin{equation}
	c(m;n) = \left[8m\left(1-\frac{m}{n}\right)\right]^{-1/2}.
\end{equation}
Generally, the first expression within the $\max$ function is useful iff $m \leq np$, while the second expression is due to~\cref{eq:reversef} and thususeful iff $m \geq np$.
So we choose $L(m;n,p$ to be the maximum of the two at any $m$.
Figure~\cref{fig:Phi} demonstrates the upper and lower bounds for $\Phi(m;n,\frac{4}{5}$.

\begin{figure}
    \centering
    \includegraphics[scale=0.5]{figs/Phi.pdf}
    \caption{Distributions $\Phi(m;n,p)$ (blue) and $\Phi^c(m;n,p)$ (orange) for $n=30$ and $p=\frac{4}{5}$.
    The upper and lower bounds on $\Phi$ are also presented. 
    The upper bound is given by~\cref{eq:ubd} and the lower bound by~\cref{eq:lbd}.
    }
    \label{fig:Phi}
\end{figure}
...
\begin{figure}
    ...
    \caption{Distributions $\Phi_+(m;n,p)/ U(m;n,p)$ (blue) and $U(m;n,p) \Phi_-(m;n,p)$ (orange) for $n=30$ and $p=\frac{4}{5}$.
    We multiply the bound in~\cref{eq:ubd} to extract the exponential behaviour of the two distributions.
