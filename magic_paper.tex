...
This provides a simple means to specify energetic details of a protocol through how much it would disturb some reference equilibrium state. The approach, when combined with relative majorization, also allows us to derive magic distillation bounds that combine both the computational measures $\epsilon,\epsilon'$, with terms that depend on the Hamiltonian of the physical system. As discussed in section (blah), from the perspective of majorization the presence of magic can be viewed as a form of non-classical free energy and so it is also of interest to make an explicit link between the amount of magic in a non-equilibrium state $\rho_S(\epsilon)$ and the actual free energy of an equilibrium state $\tau(\beta)$. 
...
\end{theorem}
The proof of this is provided in Appendix \ref{free-energy-bound-proof}, and is a generalization of the analysis for unital protocols.
...
We could also perform similar analysis for general qudits, and different choices of magic states. It might also be of interest consider other choices of reference states that may include more appropriate hardware physics, for example for photonic set-ups.

The only non-trivial assumption is the assumption that we can ignore correlations in the thermodynamic limit. This is a common situation in thermodynamics, where correlations are often sub-linear in the particle number in the macroscopic limit. One could perturb about this scenario, and make use of variational tools such as the Bogoliubov inequality \ddd{[cite]} for approximating the free energy of a system, to obtain bounds in a similar form.

\subsection{Variation of bounds with physical parameters and constraints from complete positivity}
...
\ddd{[To be tidied+blended]} We also note that one can obtain another set of general distillation bounds from the monotonicity of discrimination measures under quantum channels. For example, the $L_1$ norm is contractive under quantum channels so that 
\begin{equation}
|| \rho - \sigma||_1 \ge ||\E(\rho) - \E(\sigma)||_1,
\end{equation}
for any quantum channel $\E$. Therefore, beyond the constraint that the magic protocol corresponds to a stochastic map in the Wigner distribution, we also have constraints coming from complete-positivity and trace-preservation. For example, is $D(\rho,\sigma)$ is a contractive measure on pairs of quantum states that is also additive, in the sense that $D(\rho^{\otimes n}, \sigma^{\otimes n}) = n D(\rho, \sigma)$, then it is readily seen that within the context of the above theorem we also have,
\begin{equation}
R \le \frac{D(\rho_S(\epsilon),\tau)}{D(\rho_S(\epsilon'), \tau')},
\end{equation}
provided the denominator does not vanish. In particular we can choose the Renyi-divergences $D_\alpha(\rho ||\tau)$....or more concretely the von Neumann divergence $S(\rho ||\tau) = \tr[ \rho \log \rho - \rho \log \tau]$. However, if $\tau$ is taken to be a Gibbs state then $S(\rho ||\tau) = \beta (F[\rho] - F[\tau])$, namely the difference in free energies of the two states. Here we define
\begin{equation}
F[\rho] := \tr[H \rho] - \beta^{-1} S(\rho).
\end{equation}
  Therefore, simply from the fact that the protocol defines a channel, we obtain the bound
\begin{equation}
R \le \frac{F[\rho_S(\epsilon)] - F[\tau]}{F[\rho_S(\epsilon')] - F[\tau']},
\end{equation}
and so we can compare this with the above bounds \ddd{[Some numerics on this to compare?]}
