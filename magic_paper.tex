...
We show that the mana of a quantum state also arises as a residue in the $\alpha$--parameter for Renyi entropies, and gives the rate of divergence to $-\infty$ as we approach the Shannon entropy of a Wigner distribution (Equation (\ref{eq:mana-divergence})).
...
This means in particular that $\E(\tau^{\otimes n})$ is a symmetric state on $m$ subsystems, so by the quantum de Finetti theorem~\cite{christandl_2007} \ddd{[There is more than one seminal de Finetti paper to cite]} we have that for $m \gg 1$
\begin{equation}
...
We can go further, and show that in fact a range of Renyi entropies remain both well-defined and meaningful in the Wigner representation. The $\alpha$--Renyi entropy $H_\alpha(\p)$ is defined as
\begin{equation}
...
\end{theorem}
This monotonicity provides a meaningful extension of entropies to quasi-distributions, and the allowed values of $\alpha$ are dense in $\alpha >1$. Moreover we can prove the following result, which establishes the equivalence between Wigner negativity and the existence of negative Renyi entropy.
...
\end{theorem}
\ddd{[Perhaps remove free energy form...and allow $\frac{1}{m} D_\alpha(\rho'_m|| \tau'_m)$ general term with no assumption on correlations. ]}

For magic protocols that give unital maps, the above bound reduces to the simpler form
\begin{equation}
...
In this section, we analyse Renyi entropies on quasi-probabilities and use them to get magic distillation bounds.
...
\end{equation}
where $\alpha$ can take values of the form $2a / (2b+1)$, for non-negative integers $a,b$ to ensure that the argument of the logarithm is positive. \ddd{[Need to expand on this]}
...
\begin{equation}
	\D=\mathcal{Q}_N = \left\{ \w \in \mathbb{R}^N: \sum_{i=1}^N w_i = 1 \right\}.
\end{equation}
...
\begin{equation}
	\mathcal{Q}^\downarrow_N = \{ \w \in \mathcal{Q}_N : w_1 \ge w_2 \ge \cdots \ge w_N\}.
\end{equation}

We now have that the Schur-Ostrowski theorem~\cite{cit:marshall} provides a characterization of symmetric, Schur-convex and Schur-concave functions on quasi-distributions. \ddd{[Needs a bit more expansion]}
\begin{lemma}
	Let $f$ be a real-valued continuous function defined on $\mathcal{Q}^\downarrow_N$ that is continuously differentiable on the interior of $\mathcal{Q}^\downarrow_N$. 
Then, $f$ is Schur-convex on $\mathcal{Q}^\downarrow_N$ if and only if $\partial_k f(\w)$ is non-increasing in $k$ for all $\w \in \mathcal{Q}^\downarrow_N$, namely
\begin{equation}
	\partial_1 f(\w) \ge \partial_2 f(\w) \ge \cdots \ge \partial_N f(\w),
\end{equation}
for all $\w$ in the interior of $\mathcal{Q}^\downarrow_N$.
\end{lemma}
...
\begin{theorem}
	If $\alpha = \frac{2a}{2b-1}$ for non-negative integers $a,b$ with $a \geq b$, then $H_\alpha(\w)$ is well-defined on the set of quasi-distributions $\mathcal{Q}_N$, and moreover if $\w \succ \w'$ for two quasi-distributions $\w, \w'\in \mathcal{Q}_N$ then $H_\alpha (\w) \leq H_\alpha(\w')$.
\end{theorem}
\addtocounter{theorem}{-1}
\endgroup
\begin{proof}
...
Given a fixed $\alpha$ for which $H_\alpha(\bmz)$ is well-defined, the function $H_\alpha$ is real-valued, smooth on the interior of $\mathcal{Q}^\downarrow_N$ and symmetric in the components of $\bmz$.
...
\end{equation}
We first note that for $\alpha= \frac{2a}{2b-1}$,  with $a\ge b$, it is always true that
\begin{equation}
	\frac{\alpha}{(\alpha-1)\sum_k{w_k^\alpha}} > 0.
\end{equation}
We show that for $\alpha$ in the given form, the function $f(w) := -w^{\alpha - 1}$ is non-increasing. \ddd{[Needs improving]}
...
Conversely, suppose $\rho$ has negativity in its Wigner representation. This implies that $\sum_{\bmz} |W_\rho (\bmz)| >1$. We now consider $\sum_{\bmz} W_\rho (\bmz)^{a/b}$ for $a/b >1$ and $a$ even and $b$ odd. \ddd{[$a, b \rightarrow 2a, 2b-1$]}We have that
\begin{align}
...
\end{equation}
where $\alpha$ can take values of the form $2a/(2b+1)$ \ddd{[2b+1 or 2b-1??]}, for non-negative integers $a,b$ to ensure once again that the argument of the logarithm is positive.
...
\begin{theorem}\label{thm:H2D}
	Let $\w\in \mathcal{Q}_N$ be a quasi-distribution and $\r \in \mathbb{R}^N$ a probability distribution with positive rational entries given by $r_i = a_i/K$ for positive integers $a_i$ and $K = \sum_{i=1}^N a_i$.
	Then,
	\begin{equation}
		H_\alpha(\Gamma_{\bma}(\w)) = K - D_\alpha(\w \hspace{1pt}||\hspace{1pt} \r),
	\end{equation}
	where $\alpha = 2a / (2b+1)$ for non-negative integers $a,b$ such that $H_\alpha(\bmz)$ is well-defined for all $\bmz$.
\end{theorem}
\begin{proof}
...
\end{equation}
\end{proof}
\ddd{[Add in general divergence without product form]}
