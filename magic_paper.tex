...
\end{equation}
and solve for $\epsilon$ as a function of $\beta$. We denote the value value of $\epsilon$ for which this occurs $\epsilon_{\star}$ and find that it is equal to
\begin{equation}
	\epsilon_{\star}(\beta) := 3 - \dfrac{18}{8-e^{(E_{\rm{max}} - E_0)\beta}}.
\end{equation}
For the case $E_{\rm{max}} = E_0$, namely the negative component overlaps the highest energy state distribution, this threshold is constant in temperature and given by $\epsilon_{\star} = 3/7$. 

Otherwise, we have that the negative component does not overlap with the highest energy state and so $ E_{\rm max} - E_0 > 0$. For this regime the threshold $\epsilon_{\star} (\beta)$ is a decreasing function of $\beta$, and it achieves its maximum value $3/7$ for $\beta = 0$, namely on the unital fragment. It also attains a value $0$ for
\begin{equation}
	\beta_{\star} \coloneqq \frac{1}{E_{\rm{max}} - E_0} \ln2.
\end{equation}
For $\beta > \beta_{\star}$, $\epsilon_{\star}$ is negative, meaning that the noise level $\epsilon$ will always be higher than the threshold. \ddd{For $\beta \rightarrow \infty$ surely $\epsilon_{\star} \rightarrow 3$, and so it goes negative, but then goes positive again? I.e. there's another temperature region to consider. What am I missing here?}

We note that the threshold becomes $0$ when the difference between the maximum energy level and the level associated with the state negativity is equal to the energy extracted from a single particle Szilard's engine,
\begin{equation}
