...
\begin{proof}
	...
	The defining property of a monotone, seen in~\cref{def:mono} ensures order preservation under state projection $\Pis = \M$, where the pre-order $\prec_{\R'}$ is simply the total order $\leq$ of real numbers.
	
	Let $\Pio$ project any $\E \in \O$ onto a real non-increasing function $f$ defined by mapping $\M(\rho)$ to $\M(\E(\rho))$ for every state $\rho$.
	Then, composition of free opearations implies composition of their function projections. \nick{expand?}
...
\begin{proposition}\label{thm:subproj}
    ...
    Then, any subtheory $\R' = (\F, \O')$ of $\R$, where $\O' \subseteq \O$, is a contravariant fragment of $\R$.    
\end{proposition}
\begin{proof}
    We will establish a contravariant projection $(\Pis, \Pio)$ from $\R$ to $\R'$.
    Let the state projection $\Pis$ map all states onto themselves and suppose that $\rho_1 \prec_{\R'} \rho_2$. 
    Then, there exists $\E \in \O' \subseteq \O$ such that $\rho_1 = \E(\rho_2)$, therefore $\rho_1 \prec_{\R} \rho_2$ as well.
	
    Consider the operation projection $\Pio:\O \mapsto \O'$, defined as
    \begin{equation}
    \Pio(\E) =
    \begin{cases}
        \E,\ &\E \in \O', \\
    ...
    \end{cases}
    \end{equation}    
    This projection ensures that operational composition is contravariantly preserved.
    For any $\E_1, \E_2 \in \O'$, then $\E =\E_1 \circ \E_2 \in \O_\sigma$ as well, so $\E = \Pio(\E) = \Pio(\E_1) \circ \Pio(\E_2) = \E_1 \circ \E_2$.
    If $\E_1 \notin \O'$ or $\E_2 \notin \O'$, then $\Pio(\E) \neq \Pio(\E_1) \circ \Pio(\E_2)$.
    
    \nick{This does not hold in the case of all $\E, \E_1, \E_2 \notin \O'$. We need to update the composition condition in~\cref{def:contraproj}}
\end{proof}
