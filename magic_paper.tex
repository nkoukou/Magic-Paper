...
Our approach is through the discrete Wigner representation of quantum states, in which all states and operations can be described on a discrete phase space. Crucially we focus on magic states with negativity in their Wigner function, which is known to be a necessary condition for universality in the state-injection model. However following a thermodynamic perspective in this context raises a novel problem, namely it is impossible to define entropies for quasi-distributions. To circumvent this obstacle we make use of a more fundamental tool in thermodynamics -- majorization theory.

This construction also provides distillation bounds without relying on magic monotones -- instead of projecting the pre-order of magic states onto the real line (which is a monotone) we instead cover the pre-order of states with a collection of simpler `fragments', which admit analysis through the lens of majorization. The choice of fragment breaks the equivalence of all stabilizer states in the theory and can be used to consider adding physical constraints on the form of protocols accessible.
...
\item The invariant state $\sigma$ reflects actual hardware limitations, a biased noise model, or a statement of the ambient thermal environment for the system.
\end{enumerate}
In what follows we shall be agnostic about which of the two apply. Without loss of generality, we can always write a full-rank stabiliser state $\sigma$ in the form $\sigma =  \frac{1}{\Z_\beta} e^{-\beta H}$ for some $\beta \geq 0$ and effective Hamiltonian $H$. Our results will also depend on the free energy $F(\beta)$ of the state $\sigma$, expressed as
\begin{equation}
...
\begin{equation}
	\widetilde{\rm{W}}_{\rho_{\rm{S}}|\sigma}(\bmx) = \frac{\W[\bmx]{\rho_S}}{\W[\bmx]{\sigma}},
\end{equation}
...
\begin{figure}[h]
    ...
    \caption{\textbf{Qutrit phase space regions for $W_{\rho | \sigma}(\x)$.}
    Here the negative component of the magic state overlaps the Wigner distribution of $|0\>$. The rescaled distribution attains a single value in each of the four regions that is proportional to the value depicted in the region, see~\cref{eq:bmw_rescaled}.
    }
...
However, for $0 \leq \epsilon \leq 3/7$, we have $v \geq u$, and so since we assume that $n$ is even we need that the sum $\alpha = i+j+k$ to be even for the expression to be positive. 
...
\end{align}
If the expression $\frac{u(\epsilon)}{v(\epsilon)} e^{\beta (E_{\rm{max}} - E_s)}$ is less than $1$ then the maximum occurs at $\alpha=0$, otherwise it occurs at $\alpha = n$. For a fixed stabilizer state $\sigma$ this transition is determined by the value of the depolarising noise parameter $\epsilon$ of the noisy magic state. This transition occurs at $\epsilon = \epsilon_\star$ where
...
\begin{equation}\label{eq:first_elb_coords}
	...
	\end{cases}
\end{equation}
\ddd{[Are the factors of 3 correct in the previous equation?]} To obtain the distillation bounds from majorization we can now compute the above terms for both the input and output states. Depending on the noise parameters $\epsilon, \epsilon'$ for the initial and target states the Lorenz curve data may fall into either case $\rm{(C1)}$ or case $\rm{(C2)}$.
