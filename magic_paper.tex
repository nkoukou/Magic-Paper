...
\end{equation}
and is called the \emph{Gibbs-rescaled} distribution of $\rho$ with respect to $\sigma$.
...
Finally, we assume a range of initial noise levels, $\epsilon \leq 3/7$  \nick{higher than all threshold values in literature CITE}, so that the largest Wigner component of the 1-copy noisy magic state is negative.
...
\begin{proof}
	We provide an outline of the proof here, with all technical details included in \nick{the appendix}.\\

The Wigner distribution of $\epsilon$-noisy Strange state can be written as 
\begin{equation}
...
Let $\sigma = \frac{1}{\Z} e^{-\beta H}$ be a stabiliser state, which is given in its eigen-decomposition as 
\begin{equation}
\sigma = \frac{1}{\Z} ( e^{-\beta E_0} |\varphi_0\>\< \varphi_0| + e^{-\beta E_1} |\varphi_1\>\< \varphi_1| + e^{-\beta E_2} |\varphi_2\>\< \varphi_2| ),
\end{equation}
where $\{|\varphi_k\>\}$ are pure, orthonormal stabiliser states. Therefore the exist a Clifford operation that takes $|\varphi_k\> \rightarrow |k\>$, with the effect of permuting the negative component of $\rho_S(\epsilon)$ in the discrete phase space.
