...
% Notation consistency --- noise vs error, \ln{} -> \ln, \ln vs \log vs \log_2, italics & \emph, Aberg -> {\AA}berg, quasi-dist vs quasi-prob dist, def elbows, cumulant vs cumulative, stabilizer vs free, \p vs \w, W_\rho vs W_\rho(\bmz), \succ_r vs \succ
% UK vs US english

% Figs check --- consistency, cbars, captions, axes, iffalse
...
Theories of magic states attempt to quantify this computational element via monotones and determine how these states may be efficiently transformed into useful forms. Here, we develop a statistical mechanical framework based on majorization in which Wigner negative magic states provide non-classical free energy that is processed under Clifford circuits. While traditional free energy is not defined in the presence of negativity, we show that majorization allows us to both quantify disorder in the Wigner representation and derive magic distillation bounds. These upper bounds are shown to be tighter than other bounds, such as from mana and thauma, and can incorporate hardware physics, such as temperature dependence and physical Hamiltonians. We also show that a subset of single-shot Renyi entropies remain well-defined on quasi-distributions, are fully meaningful in terms of data processing and can acquire negative values that signal magic. We show that the mana of a magic state is the measure of divergence of these Renyi entropies as one approaches the Shannon entropy for Wigner distributions, and discuss how distillation lower bounds could be obtained in this setting. This use of majorization for quasi-distributions could find application in other studies of non-classicality, and raises novel questions in the context of classical statistical mechanics.
\end{abstract}
...
We find that the analysis in the presence of negativity is non-trivial, displays a range of features that do not appear in classical statistical mechanics, and leads to a picture of Wigner negativity in a quantum circuit being described as non-classical free energy that is processed under stochastic dynamics. This is demonstrated by Lorenz curves having a non-trivial maximum related to mana (Section~\ref{sec:lc}) and by showing that single-shot entropies for general Wigner quasi-distributions are well-defined, obey data-processing under stabilizer operations and can take on negative values (Section~\ref{section:entropies}).
...
We also show that the mana of a quantum state arises as a residue in the $\alpha$--parameter for Renyi entropies, and gives the rate of divergence to $-\infty$ as we approach the Shannon entropy of a Wigner distribution (Equation (\ref{eq:mana-divergence})).
Finally, in Section~\ref{sec:lower_bounds} we discuss how our approach could be improved and how it can be exploited to construct explicit lower bounds on distillation that take into account physical constraints.
...
In order to provide a complete decomposition of arbitrary quantum states and quantum operations we now define a complete basis of Hermitian observables that behaves naturally under the action of the Clifford group. At every point $\bmz \in \P_d$ we define the phase-point operator
...
For any quantum state $\rho$, the Wigner distribution $W_\rho(\bmz)$ is readily seen to be a $d^2$-dimensional quasi-probability distribution over $\cal{P}_d$. More precisely, $W_\rho(\bmz)$ is a bounded real-valued function on $\P_d$ with the normalisation property $\sum_{\bmz} W_\rho(\bmz) = 1$ (see~\cref{app:wigner} for details).

Any quantum operation $\E$ can also admit a Wigner representation. If $\E$ maps some quantum system $A$ of dimension $d_A$ to a quantum system $B$ of dimension $d_B$ and $\J(\E)$ is its associated Choi state~\cite{watrous_2018}, then we can define
\begin{equation}
...
Any magic state protocol will correspond to a quantum channel $\E$, and admits a Wigner representation $W_{\E}( \y | \bmz)$ that can be viewed as a transition matrix mapping phase space points $\bmz \rightarrow \y$. The representation obeys the relation 
\begin{equation}
...
However, it is important to note that this mapping to stochastic maps is not one-to-one. Not all stochastic maps on the phase space correspond to valid quantum operations. The reason is that the maps must also respect the symplectic structure of the phase space, which is an additional non-trivial constraint.

In what follows, we shall assume that we have a magic theory $\R = (\F, \O)$ in which the free states $\F$ form a closed set and are represented by non-negative Wigner functions, while the free operations $\O$ are stochastic maps in the Wigner representation. To aid our analysis of magic in this phase space setting we shall make use of majorization theory, which we describe in the next section.
...
It turns out that relative majorization is equally applicable to \emph{quasi-probability} distributions, and therefore can be applied to the Wigner representation of magic across different $\R_\sigma$. We discuss this in detail in the next section.
...
\begin{figure}[t]
    ...
    \caption{\textbf{Decomposition of a magic theory $\R$ into $\R_\sigma$ bits.} 
...
We state some generic aspects of Lorenz curves for magic states. These allow us to interpret previous magic monotones in a new light. The first result gives an extremely simple way to see that the sum-negativity/mana of a magic state is a monotone~\cite{cit:veitch2}.

\begin{theorem} Majorization in any $\R_\sigma$ implies the monotonicity of sum-negativity. 
\end{theorem}
\begin{proof}
...
\end{equation}
in the computational basis. Its distribution $W_S$ has a single negative value of $-1/3$ at $\bmz =(0,0)$ and the positive value $1/6$ at all other points. We define the $\epsilon$--noisy Strange state as
...
Due to negativity, the ordering of the components of rescaled Wigner distribution $W_{\rho_n |\sigma_n}(\bmz) \coloneqq W_{\rho_n}(\bmz)/W_{\sigma_n}(\bmz)$, for $\rho_n = \rho_S(\epsilon)^{\otimes n}, \sigma_n = \sigma^{\otimes n}$, depends on whether $-v(\epsilon)$ appears to an even or odd power. Treating the Wigner distributions as vectors, the component values $w_i$ and associated multiplicities $m_i$ in the $n$--copy case are found to be 
\begin{align}
	m_i &= 8^{i}\binom{n}{i}, \\
...
Distillation bounds can be obtained from any location of the Lorenz curve, including the region around the peak, by imposing the constraint that the Lorenz curve of the input state never dips below the curve of the output. In particular a compact constraint can be obtained by considering the initial slopes of the two Lorenz curves. 
...
We have considered Wigner distributions of magic states within a statistical mechanical setting in which it was argued that magic should be viewed as a non-classical form of free energy. This identification with free energy was done at the level of majorization constraints that appear due to stabilizer operations being described by stochastic maps in the Wigner representation. In this representation, non-equilibrium states with substantial free energy have Lorenz curves that deviate noticeably from the line $y=x$. We also found that when considered in a distillation protocol, magic could be linked to thermodynamic free energies in a non-trivial way. We can therefore ask: is it possible to make a more direct link between magic and free energy or entropy?
...
However, it turns out that the analysis for the temperature-dependent bound given by Equation (\ref{eq:rate_bounds_proof}) implicitly made use of the single-shot Renyi divergence $D_\infty (\p \hspace{1pt}||\hspace{1pt} \r)$, which is given by
\begin{equation}
...
\begin{theorem}\label{thm:HSchur} 
	If $\alpha = \frac{2a}{2b-1}$ for non-negative integers $a,b$ with $a \geq b$, then $H_\alpha(\w)$ is well-defined on the set of quasi-distributions, and moreover if $\w \succ \w'$ for two quasi-distributions $\w, \w'$ then $H_\alpha (\w) \leq H_\alpha(\w')$.
\end{theorem}
...
\begin{equation}
	D_\alpha(\p \hspace{1pt}||\hspace{1pt} \r) := \frac{1}{\alpha-1} \log \sum_i p_i^\alpha r_i^{1-\alpha},
\end{equation}
...
\begin{theorem}\label{thm:entropic_bounds} 
	...
	\end{equation}
	where $\alpha = \frac{2a}{2b-1}$ for any positive integers $a,b$ with $a \geq b$ and $\beta F_\alpha(W_\rho) := D_\alpha (W_\rho \hspace{1pt}||\hspace{1pt} W_\tau) - \log \Z$ is the $\alpha$--free energy of the Wigner distribution. \ddd{[$\Z$ needs discussion]}
\end{theorem}
...
%    \caption{$R_\alpha$ and $R_\alpha - R_{\infty}$ for the Strange state distillation for unital protocols
...
\begin{proposition}
    ...
    \begin{enumerate}
        \item[(i)]\label{en:wo1} Real-valued: $\W{\E}(\bmz) \in \mathbb{R}^{d^2} \times \mathbb{R}^{d^2}$;
...
\begin{proof}
	...
	The equivalence between (i) and (iii) follows from a similar argument.
	...
	\end{align*}
Replacing $t = (\epsilon + c - 1)/\epsilon$, we see that $t$ can attain any real value for some value of $c$, so we retrieve the property on quasi-distributions $\w, \w'$.
\end{proof}
...
\endgroup
\begin{proof}
	We denote the Wigner distributions of the states single-component vectors $\w(\rho)=W_\rho(x)$ and $ \w(\sigma)=W_\sigma(x)$. Likewise, we write $\w(\rho|\sigma) = W_{\rho|\tau}(\bmz)$.
...
\begin{align}
	...
	\alpha_k &= \<E_k| A_{\bmz_\star} |E_k\>,
\end{align}
and $\bmz_\star = \arg\!\min W_\tau(\bmz)$. The primed variables are defined similarly for the output system.
\end{theorem}
\addtocounter{theorem}{-1}
\endgroup
...
1. We have that 
\begin{equation}
	D_\infty(W_\rho || W_\tau) =\log  \max_{\bmz} \frac{W_\rho}{W_\tau},
\end{equation}
...
2. $D_\infty(W_\rho || W_\tau) = 0$ implies that $\max_{\bmz} \frac{W_\rho(\bmz)}{W_\tau(\bmz)}=1$ and the initial slope of $L_{\rho|\tau}(x)$ is $1$. From the concavity of the function this implies that the slope of $L_{\rho|\tau}(x)$ must equal $1$ throughout the interval $[0,1]$ and this, together with the definition of the Lorenz curve, implies that $W_\rho(\bmz)/W_\tau(\bmz) = 1$ for all $\bmz$. Since the Wigner representation $\rho \mapsto W_\rho(\bmz)$ is bijective this implies that $\rho = \tau$ only. The converse holds by inspection.
...
\begin{figure}[h]
    \centering
    \includegraphics[scale=0.3]{figs/D_vs_eps_a.pdf}
...
\endgroup
\begin{proof}
We have $\alpha = \frac{2a}{2b+1} \geq \frac{2b}{2b-1} > 1$ and $\alpha$ is a rational with even numerator and odd denominator, so $\sum_i x_i^\alpha > 0$ and $H_\alpha(\bmz)$ is well-defined for all quasi-distributions $\bmz$.

Given a fixed $\alpha$ for which $H_\alpha(\bmz)$ is well-defined, the function $H_\alpha$ is real-valued, smooth on the interior of $\mathcal{Q}^\downarrow$ and symmetric on any component permutation of $\bmz$.
...
To this end, we first note that, for $\alpha > 1$, $f(w)$ has no discontinuities and we can obtain the first derivative
\begin{equation}
...
\end{proof}
We note that if $\alpha =a/b$ with both $a$ and $b$ odd, then $H_\alpha$ is not well-defined for all quasi-probabilities, however if the actual set of quasi-distributions has a sufficiently bounded negativity, then $\log \sum_{\bmz} W_\rho(\bmz)^\alpha$ can still be obtained for $a$ odd, provided the sum is never negative.

If $\alpha =2a/(2b-1)$ with $\alpha < 1$, then $f(w) := \frac{-w^{\alpha-1}}{\alpha-1}$ is not monotonic. For example, if $\alpha = 2/3$, then $f(-2/6) = -4.33$, $f(-1/6) = -5.45$ and $f(+2/6) = +4.33$. Finally, we note that the set $F := \{2a/(2b-1): a,b \in \mathbb{N} \text{ and } a \geq b\}$ is dense in the reals $\mathbb{R}_{>1}$, as any rational $c/d$ with $c>d$ can be approximated by  $c2^n / (d2^n-1) \in F$ for sufficiently large $n$.
...
Conversely, suppose $\rho$ has negativity in its Wigner representation. This implies that $\sum_{\bmz} |W_\rho (\bmz)| >1$. We now consider $\sum_{\bmz} W_\rho (\bmz)^{a/b}$ for $a/b >1$ and $a$ even and $b$ odd. We have that
\begin{align}
\sum_{\bmz} W_\rho (\bmz)^{a/b} &= \sum_{\bmz} |W_\rho (\bmz)|^{a/b} \nonumber \\
...
\end{equation}
Since $\frac{1}{1-\alpha}= -\frac{b_n}{a_n} <0$ for any $n$ it follows that $H_\alpha (W_\rho) = \frac{1}{1-\alpha} \log \sum_{\bmz} W_\rho (\bmz)^\alpha  <0 $ at some point in the sequence, which completes the proof.
\end{proof}
...
\begin{equation}
	D_\alpha(W_\rho \hspace{1pt}||\hspace{1pt} W_\tau) := \frac{1}{\alpha - 1} \log \sum_{\bmz} W_{\rho}(\bmz)^\alpha W_{\tau}(\bmz)^{1-\alpha},
\end{equation}
...
\begin{theorem}\label{thm:H2D}
	...
	\end{equation}
	where $\alpha = 2c / (2d+1)$ for non-negative integers $c,d$ such that $H_\alpha(\bmz)$ is well-defined for all $\bmz$.
\end{theorem}
\begin{proof}
	For the given form of $\alpha$ and positive $r_i,\ i=1,\dots,N$, $D_\alpha(\bmz \hspace{1pt}||\hspace{1pt} \r)$ is well-defined for all $\bmz$.
...
Then, the $\alpha$-Renyi divergence $D_\alpha(W_\rho \hspace{1pt}||\hspace{1pt} W_\tau)$ is well-defined for all states $\rho$, and the following hold:
\begin{enumerate}
\item $D_\alpha(W_\rho \hspace{1pt}||\hspace{1pt} W_\tau) \ge 0$ for all quantum states $\rho$.
\item  $D_\alpha(W_\rho \hspace{1pt}||\hspace{1pt} W_\tau) = 0$ if and only if $\rho =\tau$.
\item $D_\alpha(W_{\rho^{\otimes n}} \hspace{1pt}||\hspace{1pt} W_{\tau^{\otimes n}}) = n D_\alpha(W_\rho \hspace{1pt}||\hspace{1pt} W_\tau)$ for any $n \in \mathbb{N}$.
...
\begin{align}
	D_\alpha(W_{\rho} \hspace{1pt}||\hspace{1pt} W_{\tau}) \geq D_\alpha(W_{\rho'} \hspace{1pt}||\hspace{1pt} W_{\rho'}) &= \nonumber\\
	\frac{1}{\alpha-1} \log \sum_{\bmz} W_{\rho'}(\bmz) &= 0.
\end{align}
...
\begin{theorem}
	...
	\begin{equation}
		R \leq \frac{D_{\alpha}(W_\rho \hspace{1pt}||\hspace{1pt} W_\tau)}{D_{\alpha}(W_{\rho'} \hspace{1pt}||\hspace{1pt} W_{\tau'})} = \frac{F_\alpha(W_\rho) - F_\alpha(W_\tau)}{F_\alpha(W_{\rho'}) - F_\alpha(W_{\tau'})},
	\end{equation}
	where $\alpha = \frac{2a}{2b-1}$ for any positive integers $a,b$ with $a \geq b$ and $\beta F_\alpha(W_\rho) := D_\alpha (W_\rho \hspace{1pt}||\hspace{1pt} W_\tau) - \log \Z$ is the $\alpha$--free energy. 
\end{theorem}
\addtocounter{theorem}{-1}
\endgroup
\begin{proof}
...
\begin{equation}	
	D_\alpha(W_{\rho^{\otimes n}} \hspace{1pt}||\hspace{1pt} W_{\tau^{\otimes n}}) \geq D_\alpha (W_{\rho'^{\otimes m}} \hspace{1pt}||\hspace{1pt} W_{\tau'^{\otimes m}}).\vspace{10pt}
\end{equation}

We can use the additive property to rewrite this as
\begin{equation}
	n D_\alpha(W_\rho \hspace{1pt}||\hspace{1pt} W_\tau) \geq m D_\alpha(W_{\rho'} \hspace{1pt}||\hspace{1pt} W_{\tau'}).
\end{equation}

Since $\rho' \neq \tau'$, we have $D_\alpha(W_{\rho'} \hspace{1pt}||\hspace{1pt} W_{\tau'}) > 0$, which directly leads to the bound
\begin{equation}
	\frac{m}{n} \leq \frac{D_{\alpha}(W_\rho \hspace{1pt}||\hspace{1pt} W_\tau)}{D_{\alpha}(W_{\rho'} \hspace{1pt}||\hspace{1pt} W_{\tau'})}.
\end{equation}
\end{proof}
