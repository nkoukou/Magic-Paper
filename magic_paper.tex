...
We want to calculate the coordinates of the first elbow of the $n$-copy Lorenz curve $\lc{\noisys(\epsilon)}{\gamma_\beta}$.
To do this, we first need to find the highest slope value, i.e. find
\begin{equation}
...
\begin{equation}
	\widetilde{\bmw}_{\rm{max}} = \max{\{ \nu^n, \chi^n e^{n\beta} \}}.
\end{equation}
The final optimisation depends on the temperature regime, we work in.
Specifically, $\widetilde{\bmw}_{\rm{max}} =  \nu^n$ for
\begin{equation}\label{eq:temp_regime}
	\beta \leq \ln{\left(8-\frac{18}{3-\epsilon}\right)}
\end{equation}
This value of slope corresponds to term $X^0Y^0 = 1$, so the Lorenz curve coordinates are
\begin{equation}
	(x_0, L_0) = \Big( (3Z)^{-n}, \nu^n \Big).
\end{equation}

\nick{Not good enough. For curves to intersect must have such high noise that condition~\cref{eq:temp_regime} does not hold anymore. Need to tweak.}
