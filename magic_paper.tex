...
We denote the thermal state at that temperature by $\gamma \equiv \gamma_\beta = \frac{e^{-\beta H}}{E_\beta}$, where $E \equiv E_\beta = \tr[e^{-\beta H}]$.
...
\begin{align}
	\gamma &= \frac{e^{-\beta H}}{E} = \frac{e^{-\beta \sum\limits_{k=0}^2 E_k \ketbra{k}}}{E} \\
	&= \frac{e^{-\beta E_0}}{E}\ketbra{0} + \frac{e^{-\beta E_1}}{E}\ketbra{1} + \frac{e^{-\beta E_2}}{E}\ketbra{2},
\end{align}
where $E = e^{-\beta E_0} + e^{-\beta E_1} + e^{-\beta E_2}$.
...
\begin{align}
	\W[\bmx]{\gamma} &= \sum\limits_{k=0}^2 \frac{e^{-\beta E_k}}{E}\W[\bmx]{\ketbra{k}} \\
	&= \sum\limits_{k=0}^2 \frac{e^{-\beta E_k}}{E} \delta_{x_0, k} = \frac{e^{-\beta E_{x_0}}}{dE},
\end{align}
for all $\bmx \in \pd$. 
...
\begin{table}[h]
  ...
  \begin{tabular}{c|c|c|c}
    ...
    \hline
  \end{tabular}
  \caption{Wigner components $W_{\gamma}(\bmx)$, $W_{\noisys}(\bmx)$ and the associated slope $\tilde{\bmw}$ listed in order of decreasing slope (holds for all temperatures $\beta^{-1}$).}
  \label{tab:thermal_w}
\end{table}

To construct the 1-copy Lorenz curve $\lc{\noisys(\epsilon)}{\gamma_\beta}$, we simply add up the Wigner components in order of decreasing slope $\tilde{\bmw} \coloneqq W_{\noisys}(\bmx)/W_{\gamma_\beta}(\bmx)$.
...
\begin{equation}
	\widetilde{\bmw\too{n}} = \frac{W_{\noisys\too{n}}(\bmx)}{W_{\gamma\too{n}}(\bmx)} = \left( \frac{W_{\noisys}(\bmx)}{W_{\gamma}(\bmx)} \right)\too{n} = \widetilde{\bmw}\too{n}.
\end{equation}

According to~\cref{def:lc}, to construct the $n$-copy Lorenz curve, we need to sort $\widetilde{\bmw\too{n}}$ in decreasing order and take the cumulative sum of the corresponding components $W_{\noisys\too{n}}(\bmx)$ and $W_{\gamma\too{n}}(\bmx)$.

We can therefore construst a trinomial generating function that returns the values and the multiplicities of the distinct Wigner components for a general $n$-copy $\epsilon$-noisy Strange state in a $\gamma_\beta$-fragment.
\begin{equation}
	\tilde{w}(X,Y) \coloneqq (3Z)^n (-\nu + 5\chi X + 3\chi e^\beta Y)^n
\end{equation}
There are $\frac{(n+1)(n+2)}{2}$ distinct $X^i Y^j$ terms ($i,j = 0,\dots,n$ with $i+j \leq n$) with the coefficients of these terms representing the multiplicities and values of the different ratios.

We can inductively show that the ``start of the Lorenz curve'' corresponds to $\nu^n$ for even $n$ and $\nu^(n-1) \chi e^\beta$ for 
\begin{equation}
	\beta \leq \ln{\left(8-\frac{18}{3-\epsilon}\right)}
\end{equation}
\nick{Not good enough to restrict to the start of the curve (show why)}

\nick{coordinate generating functions}


In order to construct the 2-copy Lorenz curve, we first obtain the 2-copy Wigner distribution as a convolution of the 1-copy Wigner distribution (between two identical systems A and B) and then take again the cumulative sum of the Wigner components in order of decreasing slope $W_{\noisys\too{2}}(\bmx_A \oplus \bmx_B)/W_{\gamma_\beta\too{2}}(\bmx_A \oplus \bmx_B)$.
This order, hence the first elbow depends on the temperature,
\begin{equation}
	(x_0, L_0) = 
	\begin{cases}
		\left( \frac{1}{9(2+e^{-\beta})^2}, \left(\frac{1}{3} - \frac{4}{9}\epsilon \right)^2 \right),\ &\beta \leq \ln{\left(8-\frac{18}{3-\epsilon}\right)}, \\
		\left( \frac{1}{(1+2e^\beta)^2}, \left(\frac{1}{2} - \frac{1}{6}\epsilon \right)^2 \right),\ &\beta \geq \ln{\left(8-\frac{18}{3-\epsilon}\right)}.
	\end{cases}
\end{equation}

\nick{Unfortunately, performing the same calculation as in~\cref{eq:constraint_deriv} for $\beta \leq \ln{\left(8-\frac{18}{3-\epsilon}\right)}$, yields a very loose bound.
The first elbow is not a good indicator in this case, points closer to the peak intersect first.}
...
\begin{equation}
	\W{\gamma_\beta} = \frac{1}{3E_\beta} \big( \overbrace{e^{-\beta E_0}}^3, \overbrace{e^{-\beta E_1}}^3, \overbrace{e^{-\beta E_2}}^3 \big),
\end{equation}
...
\begin{equation}
	\begin{split}
	\W{\gamma_\beta\too{2}} = \frac{1}{9E_\beta^2} \Bigg( &\overbrace{\overbrace{e^{-\beta 2E_0}}^3, \overbrace{e^{-\beta (E_0 + E_1)}}^3, \overbrace{e^{-\beta (E_0 + E_2)}}^3}^3, \\
