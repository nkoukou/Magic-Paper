...
Therefore, $\W[\bmu]{(\frac{1}{d}\id)^{\otimes n}} = \frac{1}{d^{2n}}$ for all $\bmu \in \pd[d^n]$ and the Lorenz curve ordering only depends on $\rho$ \nick{rephrase}
...
All $n$ elbows, depicted in the diagram of~\cref{fig:lccalc}, are given by the following sums, $\left( x_k, \lc{\ketbra{\rm{S}}^{\otimes n}}{\frac{1}{d}\id}(k) \right) =$
\begin{equation}\label{eq:leftelb}
	\left( \frac{1}{9^n} \sum\limits_{j=0}^{k-1} 8^{2j} \binom{n}{2j}, \frac{1}{3^n} \sum\limits_{j=0}^{k-1} 4^{2j} \binom{n}{2j} \right)
\end{equation}
for all elbows left of the maximum, $k = 1, \dots, k_\star = \floor{\frac{n}{2}}+1$ and $\left( x_k, \lc{\ketbra{\rm{S}}^{\otimes n}}{\frac{1}{d}\id}(k) \right) = (x_{k_\star}, L(k_\star)) + $
\begin{equation}\label{eq:rightelb}
	\left( \left(\frac{8}{9}\right)^n \sum\limits_{j=0}^{k' - k_\star - 1} 8^{-(2j+1)} \binom{n}{2j+1}, \left(\frac{4}{3}\right)^n \sum\limits_{j=0}^{k' - k_\star - 1} 4^{-(2j+1)} \binom{n}{2j+1} \right),
\end{equation}
for all elbows right of the maximum, $k' = k_\star + 1, \dots, n$.
...
A set of necessary and sufficient conditions for cyclic majorization has been obtained and is given as follows. \ddd{[I'll fill this out soon.]}
