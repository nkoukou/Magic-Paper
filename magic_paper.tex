...
\subsubsection{Pointwise Lorenz curve comparison}

In this subsection, we obtain explicit coordinates for all points on the Lorenz curve.

Consider the $\epsilon$-noisy, $n$-copy Strange state
\begin{equation}
    \noisysn = \left( (1 - \epsilon) \ket{\rm{S}}\bra{\rm{S}} + \epsilon \frac{1}{3}\id \right)^{\otimes n},
\end{equation}
with Wigner distribution
\begin{equation}
	\begin{split}
		\W{\noisysn} &= \left( \overbrace{\frac{1}{6} - \frac{1}{18}\epsilon, \dots, \frac{1}{6} - \frac{1}{18}\epsilon}^8, \overbrace{-\frac{1}{3} + \frac{4}{9}\epsilon}^1 \right)^{\otimes n} \\
		&= \rm{W}^+_{\noisysn} + \rm{W}^-_{\noisysn},
	\end{split}
\end{equation}
where we split non-negative and negative components into $\rm{W}^+_{\noisysn}$ and $\rm{W}^-_{\noisysn}$.

As a simple example, we split the Wigner distribution of state $\rho_{\rm{S}}(0)^{\otimes 2}$,
\begin{equation*}
	\begin{split}
    &\rm{W}^+_{\rho_{\rm{S}}(0)^{\otimes 2}} = \left( \overbrace{\left( -\frac{1}{3} \right)^2}^1, \overbrace{\left( \frac{1}{6} \right)^2, \dots, \left( \frac{1}{6} \right)^2}^{64} \right) \text{ and } \\
    &\rm{W}^-_{\rho_{\rm{S}}(0)^{\otimes 2}} = \left( \overbrace{ -\frac{1}{3} \cdot \frac{1}{6}, \dots, -\frac{1}{3} \cdot \frac{1}{6}}^{16} \right)
    \end{split}
\end{equation*}

For now, we restrict to the non-negative Wigner components $\rm{W}^+_{\noisysn}$ [i.e. the ``LHS of the Lorenz curve''] and we drop superscript $+$ for clarity.
Assume: 
\begin{itemize}
...
As a simple example, we plot the Lorenz curve of state $\rho_{\rm{S}}(0)^{\otimes 2}$,
\begin{figure}[h]
    \centering
...
\begin{equation}
    \rho(r, \epsilon, s) = \rho_{\rm{S}}(\epsilon)^{\otimes r} \otimes \left( \frac{1}{3}\id \right)^{\otimes s}.
\end{equation}
...
\begin{align}
    x_{k,\alpha, \beta} &= \left( 1-p_{k,\alpha, \beta}\right) x_{k-1} + p_{k,\alpha, \beta} x_k \\
    L_{k,\alpha, \beta} &= \left( 1-p_{k,\alpha, \beta} \right) L_{k-1} + p_{k,\alpha, \beta} L_k, \\
...
\end{equation}
This index function is bijective.

\section*{Strange state Lorenz curve comparison in the unital fragment}
...
\end{equation}
In order to get Lorenz curve constraints on this state conversion, we need to match up the indices for the two states.
We use non-primed indices for the initial state and primed indices for the target state,
\begin{align}
    i(k, \alpha) &= 1 + (\alpha-1) + \sum_{j=0}^{k-1} 8^{2j}\binom{n}{2j}, \\
    i'(k',\alpha',\beta') &= \beta' + \left[ (\alpha'-1) + \sum_{j'=0}^{k'-1} 8^{2j'}\binom{r}{2j'} \right]9^s,
\end{align}
for $i,i' = 1,2,\dots, \frac{1}{2}(7^{n} + 9^{n})$.

We need $x_{i(k, \alpha)} = x_{i'(k', \alpha', \beta')}$, so the question is: \emph{Given a triplet $(k', \alpha', \beta')$, what is the tuple $(k, \alpha)$ such that $i(k, \alpha) = i'(k',\alpha',\beta')$?}

In other words, we choose a fixed $i'(k',\alpha',\beta')$ and we are searching for $i(k, \alpha)$ such that 
\begin{equation}
	i(k, \alpha) \equiv \alpha + \sum_{j=0}^{k-1} 8^{2j}\binom{n}{2j} = i'(k',\alpha',\beta').
\end{equation}

As an example, consider the process 
\begin{equation}
\rho_{\rm{S}}(\epsilon)^{\otimes 4} \xrightarrow{\mathcal{O}_{\id/d}} \rho_{\rm{S}}(\epsilon')^{\otimes 2} \otimes \left( \frac{1}{3}\id \right)^{\otimes 2}.
\end{equation}
We want to check which Lorenz curve is higher at the first two elbows of the target state, i.e. we want to verify or reject the inequalities
\begin{align}
	L_{i(k_0, \alpha_0)} &\geq L_{i'(0, 1, 81)}, \\
	L_{i(k_1, \alpha_1)} &\geq L_{i'(1, 64, 81)},
\end{align}
where the first two multiplicities of the target state are $1$ and $64$.
The challenge is to find $k_0, \alpha_0$ such that $i(k_0, \alpha_0) = i'(0,1,81)$ and $k_1, \alpha_1$ such that $i(k_1, \alpha_1) = i'(1,64,81)$
...
\begin{theorem}\label{thm:elbows}
	...
	Let $r$ be the number of elbows of $\lc{\tau}{\sigma}(x)$ and $T \coloneqq \{x_0, x_1, \dots, x_r, x_{r+1} \}$ be the $x$-coordinates of the elbows with $x_0 = 0$ and $x_{r+1} = 1$.
	
	Then, $\lc{\rho}{\sigma}(x) \geq \lc{\tau}{\sigma}(x)$ for all $x \in [0,1]$ iff $\lc{\rho}{\sigma}(x_j) \geq \lc{\tau}{\sigma}(x_j)$ for all $j =1,\dots,r$.
\end{theorem}
\begin{proof}	
	$\lc{\rho}{\sigma}(x) \geq \lc{\tau}{\sigma}(x)$ for all $x \in [0,1]$ trivially implies $\lc{\rho}{\sigma}(x_j) \geq \lc{\tau}{\sigma}(x_j)$ for all $j =1,\dots,r$.
	
	Conversely, assume that $\lc{\rho}{\sigma}(x_j) \geq \lc{\tau}{\sigma}(x_j)$ for all $j =1,\dots,r$.
	First, note that $\lc{\rho}{\sigma}(x_0) = \lc{\tau}{\sigma}(x_0) = 0$ and $\lc{\rho}{\sigma}(x_{r+1}) = \lc{\tau}{\sigma}(x_{r+1}) = 1$.
...
\end{figure}
	...
	This argument can be made in all intervals $[x_i, x_{i+1}]$ with $i=0,\dots,r$, so the proof is complete.
\end{proof}


Returning to~\cref{eq:su_conversion}, the $r$ independent necessary constraints are
\begin{align}
\lc{\noisysn}{\frac{1}{d}\id}\left(x_{(0, m'_{0}, 9^{s})}\right) &\geq \lc{\rho_{\rm{S}}(\epsilon_{\tau})^{\otimes k}}{\frac{1}{d}\id}\left(x_{(0, m'_{0}, 9^{s})}\right), \nonumber\\
\lc{\noisysn}{\frac{1}{d}\id}\left(x_{(1, m'_{1}, 9^{s})}\right) &\geq \lc{\rho_{\rm{S}}(\epsilon_{\tau})^{\otimes k}}{\frac{1}{d}\id}\left(x_{(1, m'_{1}, 9^{s})}\right), \nonumber\\
&\vdotswithin{=} \nonumber\\
\lc{\noisysn}{\frac{1}{d}\id}\left(x_{(r, m'_{r}, 9^{s})}\right) &\geq \lc{\rho_{\rm{S}}(\epsilon_{\tau})^{\otimes k}}{\frac{1}{d}\id}\left(x_{(r, m'_{r}, 9^{s})}\right), \nonumber
\end{align}
where $x_{(0, m'_{0}, 9^{s'})}, x_{(1, m'_{1}, 9^{s'})}, \dots, x_{(r, m'_{r}, 9^{s'})} \in T \subset X$ are the locations of the elbows of $\lc{\rho_{\rm{S}}(\epsilon_{\tau})^{\otimes r}}{\frac{1}{d}\id}$ (excluding $0$ and $1$).
