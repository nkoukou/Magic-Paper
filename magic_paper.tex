...
Consider a general qutrit Hamiltonian $H$ with spectrum $(E_0, E_1, E_2)$.
...
Consider the simple scenario, where $H$ has the spectrum $(1,0,0)$.
...
\begin{table}[h]
  ...
  \begin{tabular}{c|c|c|c}
    ...
    \hline
  \end{tabular}
  \caption{Wigner components $W_{\gamma_\beta}(\bmx)$, $W_{\noisys}(\bmx)$ and their ratio $W_{\noisys}(\bmx)/W_{\gamma_\beta}(\bmx)$ listed in order of decreasing ratio (holds for all $\beta$).}
  \label{tab:thermal_w}
\end{table}
To construct the Lorenz curves, we simply take the cumulative sum of the Wigner components in order of increasing ratio. 
