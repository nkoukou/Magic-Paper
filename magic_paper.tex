...
\end{align}
where $\omega \coloneqq e^{2\pi i/d}$ is the $d$-th root of unity and addition is modulo $d$. The Hilbert space $\hd$ is associated with a phase space $\cal{P}_d \coloneqq \mathbb{Z}_d \times \mathbb{Z}_d$, where every point $\bmx \coloneqq (x, p)$ corresponds to a displacement operator, defined as
...
where 
\begin{align*}
	\bmx \coloneqq (x_1, p_1, x_2, p_2, \dots, x_n, p_n) \in \cal{P}_{d_1} \times \dots \times \cal{P}_{d_n},
\end{align*}
...
\subsection{Wigner representations for quantum states and quantum channels}\label{sec:wigner}
...
The single-qudit Hadamard gate $H$ and phase gate $S$ generate the $d$-dimensional Clifford group $\cal{C}_d$~\cite{cit:gottesman1998}.
...
Without loss of generality we can always write a state $\sigma$ as a thermal state, $\sigma = \gamma_\beta \coloneqq \frac{1}{\Z} e^{-\beta H}$ for some $\beta \ge 0$ and Hamiltonian $H$ (either effective or actual)\footnote{Technicalities arise for the case where $\sigma$ is not full rank, but this can be still described via $ \beta \rightarrow 0$ limiting process.}.
...
\nick{Also mention relative majorization somewhere}
