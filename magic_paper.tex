...
We consider the problem of purifying $\NN$ copies of a noisy strange state $\noisysN$ into a smaller number of $\tt$ copies of a less noisy strange state $\rho(\epsilon')\too{\tt}$, with $\epsilon' \le \epsilon$ and $\tt \leq \nn$. 
Since the state $\frac{1}{3} \I$ is free in this fragment, the interconversion problem is unaffected if we tensor in copies of it. 
...
\end{equation}
where all copies are even.
Since the initial state can be written in the form $\noisys(2\nn, \epsilon, 0)$, it suffices to construct the Lorenz curve $\lc{\rho}{\id/3}(x)$ for the generic noisy strange state $\noisys(\nn, \epsilon ,\tt)$.
The exact expressions for the coordinates of this Lorenz curve can take $2^3 = 8$ different forms, depending on whether the noise level $\epsilon$ is lower/higher than $3/7$, the parity of the number of copies $\NN$ is even/odd and the location relative to the curve peak is on the left hand side (LHS -- including the maximum) / right hand side (RHS).
They are given in full detail in~\cref{app:lcsu_technical}.

Here we focus on the case of LHS points on Lorenz curves of even copies and noise levels $\epsilon \leq 3/7$.
The state $\rho(2\nn, \epsilon ,2\tt)$ has a Wigner distribution that takes on values $w_\ii$, with associated multiplicities $\mlt_\ii$, where
\begin{align}
w_\ii &= \left( \frac{1}{6} - \frac{1}{18}\epsilon \right)^{2\ii}\left( -\frac{1}{3} + \frac{4}{9}\epsilon \right)^{2(\nn-\ii)} \\
m_\ii &= 8^{2\ii}\binom{2\nn}{2\ii}
\end{align}
From this, it is readily seen that $L_\sigma(x)$ has elbows on the LHS of its peak located at points $x=x_\ii$, where
\begin{equation}
x_\ii = \Phi_+ (\ii, \nn , \frac{8}{9}),
\end{equation}
with $\ii = 0,1, \dots , n$ and where $\Phi_+(i,n,p)$ is an even-power cumulant distribution function given by
\begin{equation}
\Phi_+(\ii, \nn, p) \coloneqq \sum_{\dummy=0}^{\ii} \binom{2\nn}{2\dummy} p ^{2\ii} (1-p)^{2(\nn-\ii)}.
\end{equation}
The value of the Lorenz curve at an elbow is given by
\begin{equation}
\lc{\rho}{\id/3}(x_{\ii}) \equiv L_{\ii} =  \left( \frac{5}{3} - \frac{8}{9}\epsilon\ \right)^{2\nn} \Phi_+\left(i;\nn,\frac{12-4\epsilon}{15-8\epsilon}\right),
\end{equation}
