...
\end{lemma}
\begin{proof}
	Borrowing notation from~\cref{def:lc}, let $k_\star$ be the index of the smallest non-negative component $\tilde{\bmz}_{\pi(i)}$ of $\tilde{\bmz}^\downarrow$.
	It is clear by the construction of the Lorenz curve described in~\cref{def:lc} that the maximum is achieved at
	\begin{equation}
		x_{k_\star} \coloneqq \sum_{i=1}^{k_\star} (\W{\sigma})_{\pi(i)},
	\end{equation}
	and takes the value
