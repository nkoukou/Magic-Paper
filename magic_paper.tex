...
\nick{
\begin{proof}
	The Lorenz curve of $\p$ relative to $\r$ passes through $(0,0)$ and the points $(\sum_{i=1}^k{r_{\pi(i)}}, \sum_{i=1}^k p_{\pi(i)})$ where $\pi$ is the permutation that puts $p_i/r_i$ in non-increasing order.
	By construction, the slope of the curve attains the values $p_{\pi(i)}/r_{\pi(i)}$ throughout the interval $[0,1]$, which is a non-increasing sequence due to the definition of $\pi$.
...
\nick{As a result, every Strange state Lorenz curve in the unital fragment generally contains $n$ elbows. In the case of accidental degeneracies among Wigner components, some elbows my be straightened out.}
