...
Let $\bar n = 2\floor{\frac{n}{2}}$ be equal to $n$ if $n$ is even and $n-1$ if $n$ is odd.
...
\end{equation}
Let $\Phi_+(m; n, p)$ denote the same sum but only considering the even indices, $j = 0, 2, \dots, m$ and similarly let $\Phi_-(m; n, p)$ denote the same sum but only considering the odd indices, $j = 1, 3, \dots, m$.
Then,
\begin{align}	
	\Phi_+ + \Phi_- &= \Phi \\
	\Phi_+ - \Phi_- &= \Phi^c \coloneqq \sum\limits_{j=0}^m \binom{n}{j} (-p)^j (1-p)^{n-j}.
\end{align}

\nick{$\Phi$ is bounded by the two exponentials in Wikipedia ONLY for $m\leq np$ so these bounds do not hold for $m>np$.

$\Phi^c$ is NOT bounded above by setting $m=n$ (which yields $(1 - 2p)^n$) and NOT bounded below by 0 - both due to the existence of negative terms in the sum.
It does appear however to be strictly within the open interval $(-1, 1)$.}
...
\end{align}
where $L_\star = L_+(n; n)$ is the maximum of the curve and $L_+, L_-$ indicate points on the left, right of the curve respectively for some index $m$.
