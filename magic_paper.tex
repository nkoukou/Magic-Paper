...
\end{align}
The last expression is maximised for $\alpha = 0$ ($\alpha = n$) when the expression $\frac{u}{v} e^{\beta (E_{\rm{max}} - E_0)}$ is less than or equal to $1$ (greater than $1$).
...
\end{equation}
The noise threshold $\epsilon_{\rm{th}}$ is a decreasing function of $\beta$.
It achieves its maximum value $3/7$ for $(E_{\rm{max}} - E_0)\beta = 0$, and its minimum value $0$ for
\begin{equation}
...
\end{equation}
which holds for all finite $n$ with $\tt \leq n$ and $\epsilon' \leq \epsilon \leq \epsilon_{\rm{th}}$. \nick{In fact, it holds for $0 \leq \epsilon \leq \epsilon_*$, where $\epsilon_\star$ is the noise level (lower than $\epsilon_{\rm{th}}$) at which the state conversion turns from $\rm{(C1)} \rightarrow \rm{(C1)}$ to $\rm{(C1)} \rightarrow \rm{(C2)}$. Need to adjust~\cref{fig:distill_bounds_temp} accordingly}
