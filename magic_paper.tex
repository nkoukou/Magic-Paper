...
\item The state $\sigma$ corresponds to actual hardware limitations or to biased noise model in which $\sigma$ is an equilibrium state of some kind. 
\end{enumerate}
Without loss of generality, we can always write a full-rank stabiliser state $\sigma$ in the form $\sigma =  \frac{1}{\Z_\beta} e^{-\beta H}$ for some $\beta \geq 0$ and effective Hamiltonian $H$. Our results will also depend on the free energy $F(\beta)$ of the state $\sigma$, which is defined by
\begin{equation}
...
\begin{theorem}\label{thm:stab_bounds}
	Given any theory of magic $\R = (\F, \O)$. Let $\sigma$ be any qutrit stabiliser state, and write the state as $\sigma = \frac{1}{\Z} e^{-\beta H}$ with $H$ having eigenvalues $E_0, E_1, E_2$, and where $\beta$ is an effective inverse temperature for the state. 
We define $\beta_\star = (1/kT_\star)$ through the relation
\begin{equation}
	E_{\rm max} - E_s =: kT_\star \ln 2,
\end{equation}
where $E_{\rm max} = \max\{E_k\}$ and $E_s$ is the eigenvalue of the eigenstate that overlaps the negative component of $\rho_S(\epsilon)$.  For $\beta \leq \beta_\star$ define a threshold noise level
\begin{equation}
...
Let $\displaystyle \sigma = e^{-\beta H} / \Z_\beta$ be a stabiliser state, where $\beta \geq 0$ and $H = E_0 \ketbra{\varphi_0} + E_1 \ketbra{\varphi_1} + E_2 \ketbra{\varphi_2}$ is an eigendecomposition of $H$, where we have $E_0 \le E_1 \le E_2$.
...
\begin{align}
	\W[\bmx]{\sigma} &= \sum\limits_{k=0}^2 \frac{e^{-\beta E_k}}{\Z_\beta}W_{\ketbra{k}}(x_0,x_1) \nonumber\\
	&= \sum\limits_{k=0}^2 \frac{e^{-\beta E_k}}{\Z_\beta} \delta_{x_0, k} = \frac{e^{-\beta E_{x_0}}}{3\Z_\beta},
\end{align}
where $x_0$ is the first component of $\x$, labeling one of the three vertical lines in the phase space.
...
Using this notation, the values and multiplicities of the $n$--copy distribution $\bmw(\rho^{\otimes n}_S |\sigma^{\otimes n})$ are computed in~\cref{lem:ncopycomponents} in~\cref{app:cmpairs}. The values are denoted by $w_{ijk}$ and with associated multiplicity $m_{ijk}$. The indices $i,j,k$ obey the constraint $\alpha \coloneqq i+j+k \leq n$.
...
\begin{align}
	&\bmw(\rho_{\rm{S}} | \sigma)_{\rm{max}} \coloneqq (3\Z)^{n}\times \nonumber\\
...
\begin{equation}
	...
	\end{cases}
\end{equation}
and we have $E_{\rm{max}} \ne E_s$, so the multiplicity in each case is $3^n$.

The coordinates of the first elbow are now given by
...
\end{enumerate}
The fourth case $\rm{(C1)} \rightarrow \rm{(C2)}$ is not considered as it would correspond to $\epsilon' > \epsilon$ \ddd{[True?]}, which means that the output magic state is noisier than the input state.

In each of the three scenarios, it is simple to check that the initial state's first elbow has $x_0$ coordinate closer to $0$) than that of the output state: $x_0 \leq x_0'$, see~\cref{app:first_elb_loc} for details. Therefore we obtain distillation bounds directly by comparing $L_0$ and $L_0'$.  We we do so we obtain the following distillation bounds,
