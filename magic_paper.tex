...
\end{definition}
\nick{Move the complicated last equation to appendix?}
...
In particular, mana can be re-expressed as $\mana{\rho} = \log{(2\hspace{2pt}\sn{\rho}+1)}$, where the sum-negativity ($\rm{sn}$) \nick{CITE} is the sum of the negative components in $\W{\rho}$,
\begin{equation}
...
\begin{equation}\label{eq:Skmax}
    \max_{x\in [0,1]}{\lc{\ketbra{S}^{\otimes k}} {\sigma}}(x) = \frac{1}{2}\left (1 + \left(\frac{5}{3}\right)^k \right).
\end{equation}
