...
\begin{theorem}\label{thm:free-energy}
	Consider a magic distillation protocol on $n \gg 1$ qutrits transforming noisy Strange state $\rho_S(\epsilon)^{\otimes n} \rightarrow \rho_S(\epsilon')^{\otimes m}$, under the assumptions just stated.

Let each qutrit have a Hamiltonian $H$ with energies $E_0, E_1, E_2$. Let $T =(k\beta)^{-1}$ be any characteristic temperature for the physical system in the state $\tau$, with free energy $F$. 
...
\begin{equation}
	...
	\end{cases}
\end{equation}
Then the distillation rate $R = m/n$ of the magic protocol is bounded as follows:
\begin{equation}\label{eq:rate_bounds_proof}
	...
	\end{cases}
\end{equation}
Where $F'$ is the free energy of the single qutrit output state $\sigma = e^{-\beta H'}/\Z'$ of the protocol applied to $\tau$. 
\end{theorem}
