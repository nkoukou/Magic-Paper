...
To this end, we obtain the first derivative
\begin{equation}
...
\end{equation}
where each expression is well defined because the rational exponent has even numerator and odd denominator.
\nick{This step is the reason why $\alpha =$ odd$/$odd or $\alpha < 1$ do not work. $\alpha - 2$ may be less than 0, is this a problem?}
...
\begin{table}[h]
  ...
  \end{tabular}
  \caption{\nick{NUMERICS ON SCHUR-OSTROWSKI CONDITION} Seems like $\alpha=$ even$/$odd and $\alpha > 1$ works for any combination of $w_i, w_{i+1}$ with $w_i > w_{i+1}$ and seems also that no other case works.}
  \label{tab:test}
\end{table}

\end{proof}
Note that if $\alpha < 1$ or is of the form odd$/$even, the argument of~\cref{thm:HSchur} does not hold.
However, the set $F := \{2a/(2b+1): a,b \in \mathbb{N} \text{ and } a>b\}$ is dense in the rationals $\mathbb{Q}_{>1}$ (which in turn is dense in $\mathbb{R}_{>1}$) as any rational $c/d$ with $c>d$ can be approximated by a sequence $f_n := c2^n / (d2^n+1)$ with every element $f_n$ in the set $F$. \nick{can turn into lemma}
...
\begin{proof}
	...
	We have that $(W_{\rho_n}, W_{\tau_n}) \succ W_{\rho'_{n'}}, W_{\tau'_{n'}})$, or equivalently $\Gamma_{\bma}(W_{\rho_n}) \succ \Gamma_{\bma'}(W_{\rho'_{n'}})$, where
\begin{align}
	\Gamma_{\bma}(W_{\rho_n}) &= \bigoplus_{\bmx} W_{\rho_n}(\bmx) \left(\frac{1}{KW_{\tau_n}(\bmo)}, \dots, \frac{1}{KW_{\tau_n}((d,d))} \right)\nonumber\\
	&= \frac{1}{K}\bigoplus_{\bmx}\bigoplus_{\bmy} \frac{W_{\rho_n}(\bmx)}{W_{\tau_n}(\bmy)},
\end{align}
and similarly for $\Gamma_{\bma'}(W_{\rho'_{n'}})$.
Therefore,
\begin{equation}
	H_{\alpha}(\Gamma_{\bma}(W_{\rho_n})) = \frac{1}{1-\alpha} \log_2 \left( \frac{1}{K^\alpha} \sum_{\bmx, \bmy} \frac{W_{\rho_n}(\bmx)^\alpha}{W_{\tau_n}(\bmy)^\alpha} \right),
\end{equation}
while
\begin{equation}
	D(\rho||\tau) = \frac{1}{\alpha-1} \log_2 \sum_{\bmx,\bmy} \frac{W_{\rho}(\bmx)^\alpha}{W_{\tau}(\bmy)^{\alpha-1}}.
\end{equation}
