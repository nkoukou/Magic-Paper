...
These displacement operators form the Heisenberg-Weyl group \nick{CITE} under matrix multiplication modulo phases,
...
\end{equation}
We need $L_0 \geq L_\star'$ and rearranging we get~\cref{eq:constraint_deriv_temp}.
\end{proof}
...
We substitute coordinates from~\cref{eq:first_elb_coords} appropriately in~\cref{eq:first_elb_bound} to get
\begin{equation}
...
\end{equation}
Using the properties of super-additivity and vanishing at free states of the max--thauma, we obtain
\begin{equation}
...
\begin{figure}[t]
    ...
    \caption{\textbf{Distillation bounds in the unital fragment.} Distillation bounds obtained by majorisation, mana and the line $1-\epsilon$ (tightest bound of max--thauma) are plotted for $\epsilon' = 0$, up to noise level $\epsilon = \frac{3}{7}$.
...
We substitute coordinates from~\cref{eq:first_elb_coords} appropriately in~\cref{eq:first_elb_bound} to get the following necessary conditions,
