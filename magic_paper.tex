...
% Spacing (fig positioning, sections, paragraphs)
...
Central to our construction is the representation of any quantum state or quantum operation on a system of dimension $d$ in terms of quasi-probability representations on a discrete phase space \ddd{[CITE the review on quasi-reps]}\nick{which one?}. This construction is a discrete version of Wigner representations in quantum optics.
...
We may also exploit the channel-state duality and use the normalised Choi-Jamio\l{}kowski state 
...
\end{align}
where $\bm{\bar x} = (x_1, -p_1, \dots, x_n, -p_n)$ is $\bmx$ with momentum coordinates negated.

The specific form of~\cref{eq:woperation} is chosen so that Wigner distributions of channels act as transition matrices for Wigner distributions of states, $\W{\E(\rho)} = \W{\E}\W{\rho}$.
In particular, $\cptp$ channels that map between density operators of equal dimensions and have non-negative Wigner distributions correspond to stochastic matrices, as shown in~\cref{app:wigner}
...
\nick{There are repetitions with text in introduction, this sections looks more introduction-like I think ?}
