...
% Notation consistency (R is rate not bound, noise vs error, \ln{} -> \ln, italics)
% UK vs US english
...
\end{equation}
where $\alpha$ can take values of the form $\alpha_1 / \alpha_2$, with even $\alpha_1 \geq 0$ and odd $\alpha_2 > 0$.
Note that for any quantum state $\rho$, $H_1$ in particular is not well-defined, while for any valid $\alpha$ we have that
\begin{equation}
...
\begin{figure}[h]
    ...
    \caption{\ddd{[TO DO: Set $\epsilon = 0$ and determine region of $\alpha$ such that $H <0$.]} Here, the exponent is $\alpha = even/odd$ for $1.10 \leq \alpha \leq 1.50$. First dashed contour from the right corresponds to $H_\alpha(\rho_{\rm{S}}(0)) = 0$.
    }
    \label{fig:lcs}
\end{figure}
...
\begin{equation}
	\Gamma_{\bma}(\x) := \bigoplus_{i=1}^N x_i \eta_{b_i},
\end{equation}
...
\begin{theorem}\label{thm:HSchur}
	Let $\w, \w' \in \mathbb{R}^N$ be quasi-distributions such that $\w \succ \w'$.
	Then, $H_\alpha(\w) \leq H_\alpha(\w')$, where $\alpha = \frac{2a}{2b+1}$ for any positive integers $a,b$ with $a > b$.
\end{theorem}
\begin{proof}
We have $\alpha = \frac{2a}{2b+1} \geq \frac{2b+2}{2b+1} > 1$ and $\alpha$ is a rational with even numerator and odd denominator, so $\sum_i x_i^\alpha > 0$ and $H_\alpha(\x)$ is well-defined for all quasi-distributions $\x$.

Given a fixed $\alpha$ for which $H_\alpha(\x)$ is well-defined, the function $H_\alpha$ is real-valued, smooth on the interior of $\mathcal{Q}^\downarrow$ and symmetric on permutation components of $\x$.

Consider the partial derivatives
\begin{equation}
	\frac{\partial H_\alpha}{\partial w_i} = \frac{\alpha}{(\alpha-1)\sum_k{w_k^\alpha}}(-w_i^{\alpha-1})
\end{equation}
Whenever $w_i \geq w_j$, for $\alpha$ in the given form, we always have that $\alpha - 1$ is positive and of the form odd$/$odd so \nick{expand argument}
\begin{equation}
...
\end{equation}
for any quasi-distributions $\w, \w'$.
\end{proof}
Note that if $\alpha < 1$ or is of the form odd$/$even, the argument of~\cref{thm:HSchur} does not hold.
...
\begin{theorem}
	...
	\begin{equation}
		R \leq \frac{H_\alpha \left( \id/3 \right) - H_\alpha \left( \rho \right)}{H_\alpha \left( \id/3 \right) - H_\alpha \left( \rho') \right)},
	\end{equation}
	where $\hspace{-1pt}\alpha\hspace{-1pt}$ is such that $\hspace{-1pt}H_\alpha(\sigma)\hspace{-2pt}$ is well-defined for all states $\sigma$.
\end{theorem}
\begin{proof}
	We can regularise the number of output copies in the state distillation by tensoring in $(n-n')$ copies of the (free) maximally mixed state $\id/3$, so that we instead consider the process
\begin{equation}
...
\ddd{[TO-DO AND TIDY AND CHECK]} The extension of the domain of the Renyi entropies to real values means that is it not real-valued for certain values of $\alpha$. However we can make $H_\alpha(\w)$ well-defined on the whole of $\mathcal{Q}$ if $\alpha = \frac{a}{b}$ where $b$ is an odd integer, and $a$ is an even integer. Depending on the set of quasi-distributions the quantum system supports the restriction of $a$ being even could be lifted, provided that the sum $\sum_i w_i^\alpha >0$, however we do not consider this here, and restrict to the assumption that $a$ is even.

\begin{theorem} The functions $H_\alpha(\w)$ are well-defined on $\mathcal{Q}$ for $\alpha = \frac{a}{b}$ with $a$ an even integer and $b$ an odd integer, and are Schur-concave functions.
\end{theorem}
\begin{proof}
We now make use of the Schur-Ostrowski criterion to determine when $H_\alpha$ is Schur-Convex on $\mathcal{Q}$.

We have that
\begin{align}
V_k:=-\frac{\partial H_\alpha}{\partial w_k} &= \partial_k \frac{1}{\alpha-1} \log \sum_i w_i^{\alpha} \\
&= \frac{1}{(\alpha-1) \sum_i w_i^{\alpha}}\alpha w_k^{\alpha-1}.
\end{align}
We now impose the monotonicity condition $V_1 \ge V_2 \ge V_3 \cdots \ge V_N$. This requires, setting $S= \sum_i w_i^\alpha$ for simplicity, that
\begin{align}
 \frac{1}{(\alpha-1)S}\alpha w_i^{\alpha-1}\ge \frac{1}{(\alpha-1)S}\alpha w_{i+1}^{\alpha-1},
\end{align}
for each $i=1,2,\dots, N-1$. We have that $S >0$, since $a$ is even, and assume that $\alpha >1$ so that
\begin{equation}
w_i^{\alpha-1}\ge w_{i+1}^{\alpha-1}\mbox{ for all } 1 \le i < N.
\end{equation}
For $\alpha = a/b > 1$ we have $\alpha-1 = (a-b)/b = a'/b >0$ where $a'$ is an even integer and $b$ an odd integer. We now assume that $\w \in \mathcal{Q}^\downarrow$ so that $ w_i \ge w_{i+1}$. \ddd{[Wait I'm not sure the inequalities work...]} \nick{Look at proof of~\cref{thm:HSchur} and table below}
...
\begin{table}[h]
  ...
  \end{tabular}
  \caption{Exploring in what regimes the Schur-Ostrowski criterion holds.}
  \label{tab:test}
\end{table}
