...
The distribution $W$ can always be described by a set of $D \leq d^2$ component-multiplicity pairs denoted by $\{(w_i, m_i)\}_{i=1,\dots,D}$.
\begin{definition}
...
We construct complete sets of component-multiplicity pairs and based on the distinct components of the Gibbs-rescaled distribution,
\begin{align}
	S(\noisys) &\coloneqq \{ (-\wn, 1), (\wp, 2), (\wp, 3), (\wp, 3) \} \\
	S(\gamma) &\coloneqq \Big\{ \left( \frac{e^{-\beta E_0}}{3\Z}, 1 \right), \left( \frac{e^{-\beta E_0}}{3\Z}, 2 \right), \nonumber\\
	&\hspace{23pt} \left( \frac{e^{-\beta E_1}}{3\Z}, 3 \right), \left( \frac{e^{-\beta E_2}}{3\Z}, 3 \right) \Big\} \\
	S(\noisys | \gamma) &\coloneqq \{ (-\wn 3\Z e^{\beta E_0}, 1), (\wp 3\Z e^{\beta E_0}, 2), \nonumber\\
	&\hspace{21pt} (\wp 3\Z e^{\beta E_1}, 3), (\wp 3\Z e^{\beta E_2}, 3) \}
\end{align}

There are four distinct component-multiplicity pairs for the Gibbs-rescaled distribution, so in order to use~\cref{lem:ncopycomponents} to find the components of the $n$-copy distributions, we first define an index vector
\begin{equation}
	\bmq \coloneqq (n-\alpha,i,j,k),
\end{equation}
where $i,j,k$ are non-negative integers that satisfy 
\begin{equation*}
	i+j+k \eqqcolon \alpha \leq n.
\end{equation*}
The $n$-copy 
...
\end{equation}
where $0 \leq i,j,k \leq n$ and $i+j+k \eqqcolon \alpha \leq n$.
...
\end{equation}
which holds for all finite $n$ with $\tt \leq n$ and $\epsilon' \leq \epsilon \leq \epsilon_{\rm{th}}$. \nick{In fact, it holds for $0 \leq \epsilon \leq \epsilon_*$, where $\epsilon_\star$ is the noise level (lower than $\epsilon_{\rm{th}}$) at which the state conversion turns from $\rm{(C1)} \rightarrow \rm{(C1)}$ to $\rm{(C2)} \rightarrow \rm{(C1)}$. Need to adjust~\cref{fig:distill_bounds_temp} accordingly}
