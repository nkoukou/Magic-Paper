...
There is a natural generalisation, which we call \emph{relative majorization} (also known as $d$--majorization~\cite{Veinott_1971}, or in the context of thermodynamics, thermo-majorization)\footnote{The term ``relative majorization'' is often used~\cite{Ruch_1976} to describe a further generalisation of $\bmd$-majorization, but here we use it as equivalent.}. For a fixed probability distribution $\r = (r_1, \dots, r_n)$ with positive components, we define majorization relative to $\r$ as follows:
\begin{equation}
\p \succ_{\r} \q,
\end{equation}
...
To see this, consider incorporating noisy channels in the circuit, for example dephasing channels like
\begin{equation}\label{eq:dephase}
	\Delta(\rho) = \sum_{k \in \mathbb{Z}_d} \ketbra{\phi_k}\rho\ketbra{\phi_k}
\end{equation}
in various bases $\ket{\phi_k}$.
A circuit consisting of such dephasing channels can be fully analysed in the $\sigma$--fragments of the pure basis states $\sigma = \ketbra{\phi_k}$ (see proof in~\cref{app:frag}).
Incorporating many such channels in different bases destroys the circuit symmetries, except for the invariance of the maximally mixed state.
