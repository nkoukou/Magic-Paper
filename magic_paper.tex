...
% Notation consistency (R is rate not bound, \ln{} -> \ln, italics)
% UK vs US english
...
\end{equation}
where $\epsilon$ is the depolarizing error parameter. Moreover, utilising the symmetries of the Strange state allows for any magic state $\rho$ to be processed via Clifford operations~\cite{cit:prakash,cit:prakash2} and put into this canonical form for some $\epsilon \ge 0$.
...
To simplify our analysis we will make the following physical assumption. We assume that in the asymptotic/thermodynamic limit $n,m \rightarrow \infty$ that correlations generated between subsystems are negligible. This implies that $d\mu(x)$ is peaked on a particular state $\tau'$, so $\E(\tau) = \tau^{\prime \otimes m}$. Physically, this is a natural scenario -- for example, in the context of traditional thermodynamics, it states that the output system is well-described by intrinsic variables that are encoded in $\tau'$, and correlations are sub-linear in $m$. This assumption allows us to associate a free energy per qudit of the output state, which would be more complex if non-trivial correlations were present.
...
The above bound depends on: (a) quantum computational measures $\epsilon, \epsilon'$, (b) thermodynamic quantities $F, F',$  and (c) intermediate terms $\phi, \phi'$. The intermediate terms depend on how the energy eigenbasis of the system $\{\ket{E_k}\}$ relates to the computational stabilizer basis $\{\ket{k}\}$.  In particular, the energetic term $\phi$ quantifies the degree to which the negativity in the Wigner representation can be ascribed a sharp energetic value for the given Hamiltonian of the system. Its form is similar to $F$, and it can informally be viewed as a `magic free energy' term, associated to what can informally be viewed as a `partition function' $\zeta$. The coefficients $\alpha_k$ may be negative for some $k$, when the Hamiltonian eigen-decomposes into non-stabilizer states, but the quantity $\phi$ is always well-defined, since the function $\zeta$ is a positive multiple of the stabilizer Wigner component $W_\tau(0,0)$, as seen in the proof of the theorem. The quantity $\phi$ would diverge if the state $\tau$ acquired zero Wigner components, which only occurs if $\tau$ is on the boundary of the set of Wigner-positive states.

In the case that the system's Hamiltonian $H$ has an eigenbasis consisting of stabilizer states then we show in~\cref{app:main_proof} that $\phi$ takes on a sharp, temperature-independent energy value $\phi \in \{E_0,E_1,E_2\}$. 
...
\begin{figure}[t!]
    ...
    \caption{\textbf{Temperature-dependent bounds for magic distillation.} \nick{Need to replot with one shared colorbar} Shown are two contour plots of the bound on $R(\epsilon, \beta)$ for $H = H' = \sum_{k \in \mathbb{Z}_3} k |k\>\<k|$ and $\epsilon' = 0$, where $\beta$ is the inverse temperature and $\epsilon$ the depolarising error of the input magic state. The relevant data from the spectrum of the qutrit operator $H$ enters in via $\epsilon_\star(\beta)$ and $\beta_\star$. The $\beta = 0$ line corresponds to the regime in which the magic protocol corresponds to a unital channel.
