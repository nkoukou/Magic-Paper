...
We approach this problem using tools from statistical mechanics and recent work from the overlap of thermodynamics and quantum information theory~\cite{cit:horodecki2, Brandao_2015, Vinjanampathy_2016, Goold_2016, cit:lostaglio, cit:gour}\nick{Can't find Danzig's paper anywhere}\ddd{[Remembered name incorrectly, is this one: Janzing, Dominik, et al. "Thermodynamic cost of reliability and low temperatures: tightening Landauer's principle and the second law." Put this one first in list -- people always miss it, but it is the first.] }. In particular, we shall view \emph{magic as a form of free energy}, relative to the set of stabilizer states. This perspective takes the convex set of stabilizer states as the set of ``equilibrium'' states, and magic corresponds to a form of non-equilibrium free energy that is strictly non-classical.
...
\begin{figure}[t]
	...
	A magic protocol aims to turn $n$ copies of some magic state $\rho$ to $m$ copies of some other state $\rho'$.
	Constraints on the efficiency of the protocol can be imposed by considering its action on a separate probe state $\tau$.
    } \ddd{[Not the clearest - we can zoom-chat on this.]}
    \label{fig:sketch}
\end{figure}
...
\begin{proof}Let $L_{\p|\r}(x)$ attain its maximum at $x=x_\star$. Then from the definition, the function is continuous, monotone increasing on $[0,x_\star]$, and monotone decreasing on $[x_\star, 1]$. Consider $a,b \in [0,1]$ with $a <b$ and some probability $q \in [0,1]$. If $a$ and $b$ both lie in either the interval $[0,x_\star]$ or $[x_\star, 1]$ then concavity follows immediately. Otherwise $a$ is in the first interval and $b$ in the second interval. If $w=qa + (1-q)b$ lies in the first interval then
\begin{align}
qL_{\p|\r}(a) + (1-q)L_{\p|\r}(b) &\le qL_{\p|\r}(a) + (1-q)L_{\p|\r}(x_\star)\nonumber\\
& \le L_{\p|\r}(w)
\end{align}
from monotonicity on the first interval. Otherwise $w$ lies in the second interval and
\begin{align}
qL_{\p|\r}(a) + (1-q)L_{\p|\r}(b) &\le qL_{\p|\r}(x_\star) + (1-q)L_{\p|\r}(b)\nonumber\\
& \le L_{\p|\r}(w)
\end{align}
which follows from monotonicity on the second interval. Therefore the Lorenz curve is concave on $[0,1]$.
\end{proof}
\end{lemma}
...
\end{equation}
\end{proposition}
\ddd{[I think the following is a more direct/informative proof]}
\ddd{\begin{proof} If the transformation is possible then $L_{\rho|\sigma}(x) \ge L_{\rho'|\sigma'} (x)$ for all $x$ in $[0,1]$. If we restrict to the initial linear region segment of $L_{\rho|\sigma}(x)$ joining $(0,0)$ to $(X_0,L_0)$ then we require this to be above the initial linear segment of $L_{\rho'|\sigma'}(x)$ joining $(0,0)$ to $(X_0', L_0')$. Since both these linear segments go through the origin this condition implies that the slope of the initial segment of the input Lorenz curve must be greater than or equal to the slope of the initial segment of the output Lorenz curve. This slope constraint is then equivalent to Equation \ref{eq:first_elb_bound1}. \end{proof}}
\begin{proof}
...
We now show that the relative majorization condition can be reduced to considering the elbows of the output Lorenz curve.
...
\end{proposition}
\begin{proof}	
If $\lc{\rho}{\sigma}(x) \geq \lc{\rho'}{\sigma'}(x)$ for all $x \in [0,1]$ then the condition on the elbows trivially follows.

\ddd{Conversely, assume $L_{\rho|\sigma}(x_i) \ge L_{\rho'|\sigma'}(x_i)$ for all $i=1,\dots t$. Suppose on the contrary that $L_{\rho|\sigma}(x)$ dips below $L_{\rho'|\sigma'}(x)$ at some point $y$ where $x_i < y < x_{i+1}$ for some $i$. Then this implies
\begin{align}
L_{\rho|\sigma}(x_i) &\ge L_{\rho'|\sigma'}(x_i) \nonumber \\
L_{\rho|\sigma}(y) & <L_{\rho'|\sigma'}(y) \nonumber \\
L_{\rho|\sigma}(x_{i+1}) &\ge L_{\rho'|\sigma'}(x_{i+1}).
\end{align}
However since $L_{\rho'|\sigma'}(x)$ is linear between $x_i$ and $x_{i+1}$, the above conditions imply that $L_{\rho|\sigma}(x)$ is not a concave function, which contradicts Lemma \ref{L-concave}. Therefore $L_{\rho|\sigma}(x) \ge L_{\rho'|\sigma'}(x)$ for all $x$ in $[0,1]$.
}
...
\end{proof}
The above result simplifies the computation of relative majorization constraints to looking at the elbows of the output Lorenz curve.
...
\end{align}
They are symmetric about $p=1/2$, namely $S(p) = S(1-p)$ and $\ent{p}{q} = \ent{1-p}{1-q}$. We also have the entropic bounds
\begin{lemma}\label{lem:comb_bounds}
	...
	\end{align}
\end{lemma}
The proof provided in~\cite{cit:ash} proceeds with direct calculation for the edge cases $\ell = 1,2, n-1, n-2$ and use Stirling's approximation for the remaining cases. The bounds~\cref{lem:comb_bounds} can be used to derive the following upper and lower bounds on the functions $\Phi_+, \Phi_-$.

We rewrite the functions as
\begin{equation}
...
Therefore, $\{ (W_{\bmq}, M_{\bmq}) \}_{\bmq \in C_D^{n+1}}$ contains exactly the components of $W^{\otimes n}$, completing the proof.
\end{proof}
...
\end{align}
The labels $\text{LHS}, \text{RHS}$ denote that the components contribute to the left hand side or the right hand side of the maximum (including the maximum) of the Lorenz curve.
To simplify notation, we also use multiplication modulo $(n+1)$, specifically, for any positive integer $m \leq 2n$:
\begin{equation}
	...
	\end{cases}
\end{equation}
\ddd{Where do we use this modulo $(n+1)$? I don't understand where it's appearing.}
\begin{table}[h]
...
To simplify notation, we define in terms of $n, \epsilon$ an exponentially growing term $A$ and a probability $p$, as follows:
\begin{align}
