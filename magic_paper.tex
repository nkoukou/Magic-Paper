...
\nick{Unfortunately, this is not enough. 
There are positive Wigner ditributions which correspond to invalid states i.e. they are NOT positive semi-definite.
Therefore, there are ``holes'' in the set specified by these linear constraints.
We need a stricter argument.}
...
For simplicity we shall merge this notation into $\prec_\sigma$, as there is little risk of confusion.

\begin{theorem}
    Let $\R = (\F, \O)$ be a theory of magic and suppose the state conversion $\rho \longrightarrow \tau$ is possible.
    Then, there exists a full-rank free state $\sigma \in \F$ such that  $\W{\tau} \prec_\sigma \W{\rho}$.
...
\emph{Remark 5.} We can recast the Lorenz curve into the \emph{function} $L_{\bmz|\bmd}(x)$ defined on $[0,1]$.
...
\end{theorem}
A restatement of the theorem including more equivalent conditions, along with a proof is provided in~\cref{app:major}.
...
We now see that $0\le L_{\rho |\sigma}(x) \le 1$ for all $x$ if and only if $\rho$ is a positive Wigner state. Moreover, the area of the function $L_{\rho|\sigma}$ above the line $y=1$ is a resource monotone in this fragment. \ddd{[expand on this -- relate to mana?]}
...
\end{equation}
It is a monotone function of the $\ell_1$-norm of state negativity~\nick{CITE} that satisfies the additivity condition.
...
\begin{proof}
	...
	State order is covariantly preserved due to the defining property of a monotone, seen in~\cref{def:mono}, where the pre-order $\prec_{\R'}$ is simply the total order $\leq$ on $\mathbb{R}$. 
	...
	Then, $\M(\rho):=f(\Pi_s(\rho))$ provides a numerical value for each $\rho$ that obeys the defining monotone condition.
...
\end{proof}
\nick{The above proposition is not relevant to $\sigma$-fragments where ``for any $\E_1 \not \in \O_\sigma$ and any $\E_2 \in \O_\sigma$'' we sometimes have $\E_1 \circ \E_2 \in \O_\sigma$ as well as $\E_2 \circ \E_1 \in \O_\sigma$ and sometimes $\E_1 \circ \E_2 \notin \O_\sigma$ as well as $\E_2 \circ \E_1 \notin \O_\sigma$} \ddd{[Is this true? Act the combination of channels on $\sigma$. If it is a fixed point of only one of the channels then the composition cannot have it as a fixed point...thus the combination is in $\O_\sigma$ if and only if each of the channels is in $\O_\sigma$...right?]}
...
\end{equation}
removes coherent phases in the computational basis and therefore only leaves invariant mixtures of computational basis states.
If the circuit simply consists of such dephasing channels, the whole analysis could be restricted in the $\sigma$--fragments for pure computational basis states $\sigma$.
...
Lorenz curves provide a very efficient method of numerically checking if a certain state conversion is impossible within a $\sigma$--fragment by exploiting the equivalence with $d$--majorization stated in~\cref{thm:dmajor}.
Such methods are often more conclusive than magic monotones as we discuss in section~\cref{sec:scmana}.


\subsection{Mana and Majorization}\label{sec:scmana}
...
\end{equation}
\ddd{[We can probably put upper and lower bounds on this value fairly readily. E.g. using known sum rules. Also -- bad choice of variable $k$, using it as index and also copy number. I think $L_{\rho|\sigma}$ is a neater formulation, since you then package things into a single object -- a function.]}
...
\end{theorem}
\begin{proof}
    The maximum of the Lorenz curve of a state $\rho$ is independent of the $\sigma$--fragment
    
    The Lorenz curve maximum can then be expressed monotonically in terms of mana,
    \begin{equation}
