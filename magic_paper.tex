...
\begin{figure}
    ...
    \caption{\textbf{Variation of distillation bounds with Hamiltonian.} \nick{ We demonstrate non-trivial variation of our bound in Equation~\ref{eq:rate_bounds_proof} with repect to the system Hamiltonian. To this end, we fix $(\epsilon, \epsilon', \beta) = (0.1, 0.0, 0.2)$ and look at a family of Hamiltonians for the qutrit systems, given by $H = A_{\bmo}$ and $H' = (1-p-q)A_{\bmo}+pA_{(1,2)}+q{\rm{diag}}(0,1,2)$ for varying probabilities $p, q$. NEED TO DISCUSS VALUE $1.01$}
    }
    \label{fig:rvsa}
\end{figure}
...
\end{theorem}
\nick{Notation $D^{\rm reg}_\alpha( \rho', \tau')$ is a little clunky. What are $\rho'$ and $\tau'$ ? Maybe at least use $D^{\rm reg}_\alpha( \rho', \tau'_m)$ as in the presence of correlations marginal state $\tau'$ is not well defined}
...
If correlations in $\tau'_m$ between subsystems can be neglected then \nick{we can write $\tau'_m = \tau'^{\otimes m}$ for some sharp marginal state $\tau'$} and the regularised divergence reduces to
\begin{equation}
...
\begin{figure}[t!]
    ...
    \caption{\textbf{R\'{e}nyi entropy $H_\alpha$ of $\epsilon$-noisy Strange state.} \nick{$H_\alpha$ is increasing in $\alpha$ and $\epsilon$. The first dashed contour from the right corresponds to $H_\alpha(\rho_{\rm{S}}(\epsilon)) = 0$. In particular, $H_\alpha(\rho_{\rm{S}}(0)) = 0$ corresponds to $\alpha \approx 1.31$.}
    }
    \label{fig:H}
\end{figure}
