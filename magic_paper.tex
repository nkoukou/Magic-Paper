...
\begin{figure}[t]
    ...
    Examples of magic theories ($\so$: Stabilizer operations, $\Omax$: Completely positive-Wigner-preserving operations) involve operations within the yellow regions, with all other established magic theories between these two.
...
\begin{definition}[\textbf{Lorenz curve}]\label{def:lc}
    Let $\bmz \in \reals^d$.
    Let $\bmd \in \reals^d$ be a vector with positive components, $\pi$ a permutation mapping $(z_i/d_i) \mapsto (z_\pi(i)/d_\pi(i)) \coloneqq (z_i/d_i)^\downarrow$ for all $i=1,\dots,d$ and $D = \sum_{i=1}^d d_i$.
    ...
    \end{equation}
    The Lorenz curve of vector $\bmz$ with respect to $\bmd$ is then $\lc{\bmz}{\bmd}(x)$ for all $x \in [0,1]$.
\end{definition}
\nick{The discrete formulation is more useful in the whole discussion that follows, but the functional formulation is a little more elegant - what to choose?} \ddd{[Obviously for computing things the discrete version is best, but we want to keep the narrative clear for the unfamiliar reader. We can put the fine-print details in the appendix, but use the more elegant function expressions in the main text, of course explaining the link in simple terms. ]}
Components $x_k$ are rescaled by $D$ so that comparison of curves with unequal dimensions is possible.
In fact, one can see that the Lorenz curves $\lc{\bmz}{\bmd}$ and $\lc{\bmz \otimes \bmd}{\bmd \otimes \bmd}$, where $\otimes$ denotes the Kronecker product, are identical.
Furthermore, a Lorenz curve $\lc{\bmz}{\bmd}(x)$ is always concave in $x$, since it consists of $d$ line segments each with slope $D(\frac{z_i}{d_i})^\downarrow$ for $i=1,\dots,d$ which is non-increasing by construction.
...
Therefore, it is clear that $0 \leq \lc{\rho}{\sigma}(x) \leq 1$ for all $x \in [0,1]$ if and only if $\rho$ is a positive Wigner state.
...
If the circuit simply consists of such dephasing channels, the whole analysis can be restricted in the $\sigma$--fragments for pure computational basis states $\sigma$ \nick{as seen in a theorem in the appendix}.
...
Consider a general magic state distillation process, \nick{Generalise discussion to any state conversion?}
\begin{equation}\label{eq:distprocess}
    \rho^{\otimes k} \longrightarrow \tau,
\end{equation}
where $k$ noisy copies of magic state $\rho$ are converted to a single-copy magic state $\tau$.

It is shown in~\cref{app:major} that mana is an additive monotone in every $\sigma$--fragment under $d$--majorization, so it provides a bound for the process,
\begin{equation}
    \mana{\rho} \geq \frac{1}{k} \mana{\tau}.
\end{equation}

A new bound can be obtained in every $\sigma$--fragment by comparing the Lorenz curves of the initial and target states,
...
\begin{equation}\label{eq:Skmax}
    \max_j{L_j \left( \ketbra{S}^{\otimes k} \bigm| \sigma \right)} = 1 + \left(\frac{4}{3}\right)^k \sum\limits_{j: 1 \leq 2j+1 \leq k} 4^{-(2j+1)} \binom{k}{2j+1}.
\end{equation}
\ddd{[Why doesn't the following give the sum in closed form:]}
Let
\begin{equation}
S(a) :=(1+a)^k= \sum_{n=1}^k a^n \binom{k}{n},
\end{equation}
then we have
\begin{align}
S(a) - S(-a) &= (1+a)^k - (1-a)^k \nonumber \\
&= 2\sum_{n=1, n \mbox{ odd}}^k a^n \binom{k}{n}.
\end{align}
Thus
\begin{equation}
\sum_{n=1, n \mbox{ odd}}^k a^n \binom{k}{n} = \frac{1}{2} \left [  (1+a)^k - (1-a)^k \right ].
\end{equation}
Putting in $a = 1/4$ gives
\begin{equation}
\sum_{n=1, n \mbox{ odd}}^k \frac{1}{4^n} \binom{k}{n} = \frac{1}{2} \left [  (5/4)^k - (3/4)^k \right ].
\end{equation}
If true, then we'd have
\begin{equation}
 \max_x L_{S|\sigma}(x) = \frac{1}{2} \left (1 + \left(\frac{5}{3}\right)^k \right ).
\end{equation}
\ddd{[Am I missing something here?]}
...
At any noise level $\epsilon \leq \frac{3}{4}$, the Wigner distribution $\W{\rho_{\rm{S}}(\epsilon)}$ contains negativities and the state can be purified so as to obtain a single-copy state with sufficiently low $\epsilon$.
...
In fact, we can more generally claim that mana bounds are always weaker than bounds provided by Lorenz curve comparison for any magic state interconversion.
\begin{theorem}\label{thm:bounds}
    Given a magic state conversion $\rho \longrightarrow \tau$, $\W{\sigma}$--majorization provides a stricter bound than mana in any $\sigma$--fragment.
\end{theorem}
\begin{proof}
    The maximum of the Lorenz curve of a state $\rho$ is independent of the $\sigma$--fragment due to~\cref{lem:lcmax}, and can be expressed monotonically in terms of mana,
    \begin{equation}
        \max_k{L_k(\rho | \sigma)} = 1 + \sum_{\bmz: \W[\bmz]{\rho}<0} \abs{\W[\bmz]{\rho}} = \frac{1}{2} \left( 1 + e^\mana{\rho} \right).
    \end{equation}
    Therefore, the $\W{\sigma}$--majorization condition
    \begin{equation}
    	L_k(\rho | \sigma) \geq L_k (\tau | \sigma),\ k=1,\dots,d^2
    \end{equation}
    implies the mana condition $\mana{\rho} \geq \mana{\tau}$.
    
    \ddd{[This is a bit too short]}\nick{I have added~\cref{lem:lcmax}}\ddd{[Lemma 8 should sit next to this. :)]}
\end{proof}

\subsection{Magic in thermal fragments}

\nick{$\rcu$ and Noisy channels preserve $\frac{1}{d}\id$}

\nick{Can extend to general thermal states}
