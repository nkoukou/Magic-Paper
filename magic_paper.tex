...
Theories of magic states attempt to quantify this computational element via monotones and determine how these states may be efficiently transformed into useful forms. Here we develop a statistical mechanical framework in which Wigner negative magic states provide non-classical free energy that is processed under Clifford circuits. While traditional free energy is not defined in the presence of negativity, we show that majorization allows us to both quantify disorder in the Wigner representation and derive magic distillation bounds. These upper bounds are shown to be tighter than other bounds, such as those from mana and thauma, and moreover incorporate hardware physics, such as temperature dependence. We show that a sub-set of single-shot Renyi entropies remain well-defined on quasi-distributions, are fully meaningful in terms of data processing and can acquire negative values. We also show that the mana of a magic state is exactly the coefficient of divergence of Renyi entropies as one approaches the Shannon entropy for Wigner distributions, and discuss how distillation lower bounds could be obtained in this setting. This use of majorization for quasi-distributions could find application in other studies of non-classicality, and raises novel questions in the context of classical statistical mechanics.
\end{abstract}
...
\nick{REFs not already cited and that I think are relevant:}
\begin{enumerate}
	\item ``Excluding joint probabilities from quantum theory''~\cite{Allahverdyan_2018}
	\item ``Quantum computation with realistic magic-state factories''~\cite{Ogorman_2017}
	\item ``Magic-State Distillation in All Prime Dimensions Using Quantum Reed-Muller Codes''~\cite{CampbellAnwar_2012}
	\item ``Resource optimized quantum architectures for surface code implementations of magic-state distillation''~\cite{holmes_resource_2019}
	\item ``Quantum advantage in postselected metrology''~\cite{arvidsson_2020}
	\item ``Quasiprobability behind the out-of-time-ordered correlator''~\cite{halpern_2018}
	\item ``Quantum Fluctuation Theorems, Contextuality, and Work Quasiprobabilities''~\cite{Lostaglio_2018}
	\item ``Quasiprobability Distribution for Heat Fluctuations in the Quantum Regime''~\cite{Levy_2020}
	\item ``A One-Way Quantum Computer''~\cite{cit:raussendorf}
	\item ``Measurement-based quantum computation on cluster states''~\cite{Raussendorf_2013}
	\item ``Contextuality as a Resource for Models of Quantum Computation with Qubits''~\cite{Vega_2017}
	\item ``On the extremal points of the $\Lambda$-polytopes and classical simulation of quantum computation with magic states''~\cite{okay2021extremal}
\end{enumerate}
...
To address this, concrete distillation protocols have been developed, such as the Bravyi-Haah protocol that provides a quadratic reduction in noise per cycle~\cite{Bravyi2012}. Such distillation rates have been improved in more recent works~\cite{Hastings2018, Litinski_2019, Krishna2019, cit:prakash}. There is also analysis of magic protocols from the perspective of magic monotones, which provide upper bounds on distillation rates and find application in the analysis of gate synthesis~\cite{Campbell_2017, Howard_2017, Prakash_2018}. These frameworks for magic view magic states as resource states with respect to a natural class of quantum operations that are considered cheap, or free, such as stabilizer operations~\cite{Gour_2019, cit:ahmadi, cit:seddon, Wang_2019}. Beyond quantum computing, recent work has also looked at how magic can be used to analyse many-body physics~\cite{Sarkar_2020} and conformal field theories~\cite{White_2021}.
...
A number of magic theories exist, where magic states are computational resource states with respect to a natural class of quantum operations that are considered free~\cite{Gour_2019}. One very natural class of free operations are those obtained from Clifford operations, measurements and the ability to discard quantum systems. However, there are also other candidates~\cite{cit:ahmadi, cit:seddon, Wang_2019}.
...
\end{definition}
This notion provides a simple way to break up any theory into smaller, more manageable parts. Moreover, the union over all sub-theories returns the parent theory, so we are not discarding any information by breaking up the theory in this way. We make this statement precise as follows.
...
\subsection{Basic monotones for magic distillation for a particular $\R_\sigma$}
...
\begin{lemma} Relative majorization in any $\R_\sigma$ implies the monotonicity of sum-negativity. 
\end{lemma}
\begin{proof}
...
\begin{figure}[t]
    ...
    \caption{\textbf{Distillation bounds for unital protocols.} Distillation bounds obtained by the first elbow majorisation constraint $R_\infty$, mana and max--thauma $\theta_{\rm{max}}$ are plotted for $\epsilon' = 0$, up to error $\epsilon = 3/7$. Also shown is the bound $R_{10}$ which comes from single-shot entropies, which we discuss in Section~\ref{section:entropies}. Numerical bounds $R_{\rm{num}}$ are obtained by using the full set of Lorenz curve constraints. Each majorisation constraint provides stricter bounds than the mana and max--thauma bounds for noisy Strange states.
    }
...
\newpage
\section{Well-defined \& monotonic  entropies for quasi-distributions}\label{section:entropies}
...
This motivates defining the following \emph{Wigner divergence} $D_\infty(\rho, \tau) := D_\infty (W_\rho(\x) || W_\tau (\x))$ on all quantum states. In~\cref{app:lcst_technical}, we show that this divergence has the following natural properties that are derived via relative majorization.
...
\item For any quantum state $\rho$, and any free operation $\E$ such that $\E(\tau)$ is in the interior of $\F$, we have $D_\infty (W_\rho || W_\tau) \ge D_\infty (W_{\E(\rho)}|| W_{\E(\tau)})$.
\end{enumerate}
\end{theorem}
...
\end{equation} 
with $\epsilon \rightarrow 0^+$ through a sequence of rational values $(a-b)/b$. However we recall that mana is defined as $\M(\rho) = \log \sum_{\x} |W_\rho(\x)|$, and so we obtain a second new description of mana: \emph{it is the coefficient of divergence of the Wigner Renyi-entropy} as we approach the limiting Shannon entropy $H(\w)$. 

We also note that the Renyi entropy can be viewed as a $q$--deformation of the Shannon entropy~\cite{baez2011renyi}, and $H_\alpha(\p)$ can be related to the $q$--derivative of free energy. More precisely, it can be shown that
\begin{equation}
	H_{\frac{T}{x}} = \frac{1}{x-T} (F(x) - F(T)),
\end{equation}
\nick{Do you mean $H_{\frac{T}{x}}(\w)$? What is $F(x)$ for a number $x$ ?}
where $F(x)$ is an effective free energy at temperature $x$. If we attempt to push this to the present setting, the presence of Wigner negativity would correspond to a divergence in the first derivative of this effective free energy as $x \rightarrow T$. One might speculate if the presence of magic in Clifford circuits admits a sensible interpretation in terms of a zeroth-order phase transition. However since variation in $\alpha$ is no longer smooth ($\alpha$ is restricted to a dense set of rational values) technical obstacles exist and we do not pursue this speculative line here, but just note that other works exist in which computational transitions are considered in relation to phase transitions.

Every entropy is best understood as a relative entropy~\cite{Gour_2020} and so we now state the extension to Renyi divergences.
...
\end{proof}
The set $F := \{2a/(2b-1): a,b \in \mathbb{N} \text{ and } a \geq b\}$ is dense in the rationals $\mathbb{Q}_{>1}$, and hence the reals $\mathbb{R}_{>1}$, as any rational $c/d$ with $c>d$ can be approximated by  $c2^n / (d2^n-1) \in F$ for suficiently large $n$.
