...
\end{enumerate}
\end{theorem}
See Appendix~\ref{app:lcst_technical} for the proof of this. 
...
The $\alpha$--Renyi divergence $D_\alpha(\p)$ is defined as
\begin{equation}
	D_\alpha(\p|\r) := \frac{1}{\alpha-1} \log \sum_i p_i^\alpha r_i^{1-\alpha},
\end{equation}
where $\p$ is a quasi-distribution, $\r$ a probability distribution with positive components and $\alpha$ takes the same values as for $H_\alpha$.
...
\begin{equation}
	H_0(W_\rho) = 2\log_2 d = H_\alpha \left( W_{\id/d} \right),
\end{equation}
where $d$ is the dimension of $\rho$.

For $\rho = \rho_{\rm{S}}(\epsilon)$, we get
\begin{align}
	&H_{\alpha}\left(W_{\rho_{\rm{S}}(\epsilon)}\right) = \frac{1}{1-\alpha}\log_2 \left[ 8\left( \frac{1}{6} - \frac{1}{18}\epsilon \right)^\alpha + \left(-\frac{1}{3} + \frac{4}{9}\epsilon \right)^\alpha \right] \nonumber\\
	&= \frac{\alpha}{\alpha-1}\log_2 18+ \frac{1}{1-\alpha}\log_2 \left[ 8(3-\epsilon)^\alpha + (6-8\epsilon)^\alpha \right]
\end{align}
...
If $\alpha =$ odd$/$odd, then $f(w)$ is not monotonous because the sign of $f'(w)$ would depend on the sign of $w$.

If $\alpha =$ even$/$odd and $\alpha < 1$, then $H_\alpha$ is Schur-convex.
}
...
\begin{equation}
	D_\alpha(W_\rho || W_\tau) := \frac{1}{\alpha - 1} \log_2 \sum_{\bmx} W_{\rho}(\bmx)^\alpha W_{\tau}(\bmx)^{1-\alpha},
\end{equation}
where $\alpha$ can take values of the form $2a / (2b+1)$, for non-negative integers $a,b$ to ensure that the argument of the logarithm is positive.
...
\begin{align}
	...
	\frac{1}{\alpha-1} \log_2 \sum_{\bmx} W_{\rho'}(\bmx) &= 0.
\end{align}
