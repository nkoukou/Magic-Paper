...
\begin{figure}[h]
    ...
    The slope is positive in regions $X,Y,Z$, but attains a different value in each region and the slope is negative in region $1$.
    The distribution values and multiplicites (i.e. the numpber of phase space points contained in the region) can be read off as the term coefficients in~\cref{eq:g1}.
    }
    \label{fig:pd_split_thermal}
\end{figure}
In each region, we want to keep track of the corresponding values and multiplicities of Wigner distributions $\W{\noisys}, \W{\gamma}$ and Gibbs-rescaled distribution $\widetilde{\rm{W}}_1$. 
...
\begin{align}\label{eq:g1}
	G_1(\noisys) &\coloneqq -\wn + 3\wp X + 3\wp Y + 2\wp Z \\
	\begin{split}
	G_1(\gamma) &\coloneqq (3\Z)^{-1} \big( e^{-\beta E_2} + 3e^{-\beta E_0} X \\
	&\hspace{49pt} + 3e^{-\beta E_1} Y + 2e^{-\beta E_2} Z \big) 
	\end{split}
	\\
	\begin{split}
	G_1(\noisys|\gamma) &\coloneqq (3\Z) \big( -\wn e^{\beta E_2} + 3\wp e^{\beta E_0} X \\
	&\hspace{41pt} + 3\wp e^{\beta E_1} Y + 2\wp e^{\beta E_2} Z \big)
	\end{split}
\end{align}
...
\end{equation} 
For example, region $YZ$ consists of all points expressed as the direct sum $\bmy \oplus \bmz$, where $\bmy \in Y$ and $\bmz \in Z$. \nick{abuse of notation: I use X,Y,Z as sets and as variables that represent those sets}

The $n$-copy Wigner distributions, and therefore the $n$-copy Gibbs-rescaled distribution can be written as self-convolutions,
\begin{align}
	\W{\noisys\too{n}}(\bmx) &= \W{\noisys}(\bmx)\too{n} \\
	\W{\gamma\too{n}}(\bmx) &= \W{\gamma}(\bmx)\too{n} \\
	\begin{split}
	\widetilde{\rm{W}}_{\noisys\too{n}|\gamma\too{n}}(\bmx) &= \widetilde{\rm{W}}_n(\bmx) = \frac{\W{\noisys\too{n}}(\bmx)}{\W{\gamma\too{n}}(\bmx)} \\
	&= \left( \frac{\W{\noisys}(\bmx)}{\W{\gamma}(\bmx)} \right)\too{n} = \left( \widetilde{\rm{W}}_1(\bmx)\right) \too{n}.
	\end{split}
\end{align}
...
\begin{align}\label{eq:gn}
	...
	&(3\Z)^{-n}\hspace{-10pt} \sum\limits_{\substack{i,j,k = 0,\dots,n \\ \alpha \leq n}}\hspace{-10pt} m_{ijk} \Big[ e^{-\beta ( i E_0 + j E_1 + k E_2 )} e^{-\beta (n-\alpha)E_2} \Big] X^i Y^j Z^k, \nonumber
	\\[10pt]
	&G_n(\noisys|\gamma) \coloneqq G_1(\noisys|\gamma)^n = \\
	&(3\Z)^{n}\hspace{-10pt} \sum\limits_{\substack{i,j,k = 0,\dots,n \\ \alpha \leq n}}\hspace{-10pt} m_{ijk} \Big[ (-\wn)^{n-\alpha} \wp^{\alpha} e^{\beta (n-\alpha)E_2} \times \nonumber\\[-15pt]
...
\begin{equation}
	m_{ijk} \coloneqq \frac{n!}{i!j!k!(n-\alpha)!} 3^i 3^j 2^k.
\end{equation}
...
\begin{equation}\label{eq:max_slope}
	\widetilde{W}_{\rm{max}} \coloneqq \max\limits_{i,j,k}{\Big\{ (-\wn)^{n-\alpha} \wp^{\alpha} e^{\beta (n-\alpha)E_2} e^{\beta ( i E_0 + j E_1 + k E_2 )} \Big\}}
\end{equation}
...
Given a value for the sum $\alpha$, the term $\wn^{n-\alpha} \wp^{\alpha} e^{-\beta (n-\alpha)E_2}$ is fixed, so the expression is maximised by setting the coefficient of the highest energy $E_{\rm{max}}$ equal to $\alpha$.
Hence, we have
\begin{align}
	\widetilde{W}_{\rm{max}} &= \max\limits_{\substack{\alpha = 0,2, \\ \dots,n-2,n}}{\Big\{ (-\wn)^{n-\alpha} \wp^{\alpha} e^{\beta (n-\alpha)E_2} e^{\beta \alpha E_{\rm{max}}} \Big\}} \\
	&= \max\limits_{\substack{\alpha = 0,2, \\ \dots,n-2,n}}{\Big\{ \wn^n e^{\beta n E_2} \left( \frac{u}{v} e^{\beta (E_{\rm{max}} - E_2)} \right)^{\alpha} \Big\}}
\end{align}
The last expression is clearly maximised for $\alpha = 0$ ($\alpha = n$) when the expression $\frac{u}{v} e^{\beta (E_{\rm{max}} - E_2)} \leq 1$ ($>1$).
Therefore, we have
\begin{equation}
\widetilde{W}_{\rm{max}} =
	\begin{cases}
		\wn^n e^{n\beta E_2},\ &\beta \leq \frac{1}{E_{\rm{max}} - E_2}\ln{\left( 8-\frac{18}{3-\epsilon} \right)} \\
		\wp^n e^{n\beta E_{\rm{max}}},\ &\beta > \frac{1}{E_{\rm{max}} - E_2}\ln{\left( 8-\frac{18}{3-\epsilon} \right)}
	\end{cases}
\end{equation}
