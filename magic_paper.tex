...
\end{equation}
Now it is shown in~\cite{ruch_mixing_1978} relative majorization between pairs of distributions occurs if and only if the associated Lorenz curves never cross. Therefore we can express this condition in terms of the Lorenz curves of the quasi-probability distributions $\w$ and $\w'$ relative to $\r$ and $\r'$ respectively.
...
\label{sec:stab}
\ddd{[Various smoothings needed to blend with new emphasis]}
%
...
%\end{enumerate}
%In what follows we shall be agnostic on the interpretation. 

Without loss of generality, we can always write a full-rank stabiliser state $\sigma$ in the form $\sigma =  \frac{1}{\Z_\beta} e^{-\beta H}$ for some $\beta \geq 0$ and effective Hamiltonian $H$. Our results will also depend on the free energy $F$ of the state $\sigma$, expressed as
\begin{equation}
	F \coloneqq \tr[H \sigma] - \beta^{-1}S(\sigma) = -\beta^{-1}\log{\Z_\beta}.
\end{equation}
Here $S(\sigma)$ is the von Neumann entropy $S(\sigma) \coloneqq -\tr[\sigma\log{\sigma}]$. \nick{Note that under interpretation (1) above, this is an abstract entropic measure of the stabilizer state, while, under interpretation (2), it corresponds to the actual thermodynamic free energy of the equilibrium state.}
%
...
Given this context, we now provide the following result on bounding the distillation rate $R = R(\epsilon, \epsilon', \beta) \coloneqq \frac{n'}{n}$ of the process in\nick{~\cref{eq:stdist}}. We shall write $R(\epsilon, \beta) = R(\epsilon, \epsilon'=0, \beta)$ for the limiting case of distilling a pure magic state.
...
%\begin{equation}
%	R(\epsilon,\beta) \leq 1 + \frac{\ln{\left( 1 - \frac{4}{3}\epsilon \right)}}{\beta (E_s - F)}.
%\end{equation}
%If $\beta \leq \beta_{\star}$ and $\epsilon_{\star} < \epsilon$, then
%\begin{equation}
%	R(\epsilon, \beta) \le 1 + \frac{\ln{\left(1-\frac{1}{3}\epsilon \right)} - (E_{\rm max} - E_s)(\beta_{\star} - \beta)}{\beta (E_s - F)}.
%\end{equation}
%Otherwise if $\beta > \beta_{\star}$,
%\begin{equation}
%	R(\epsilon, \beta) \leq  1+ \frac{\ln{\left(1-\frac{1}{3}\epsilon \right)}}{-\ln{2} + \beta (E_{\rm max} - F)}.
%\end{equation}
%In the above expressions $F$ is the effective free energy of the stabilizer state.
%\end{theorem}

\ddd{[Put assumptions here. Needs more polish+checking.]} We now consider a magic distillation protocol on multiple identical qutrits sending 
\begin{equation}
\rho_S(\epsilon)^{\otimes n} \longrightarrow \rho_S(\epsilon')^{\otimes Rn}
\end{equation}
with $n \rightarrow \infty$. Let each qutrit have a Hamiltonian $H$ with energies $E_0, E_1, E_2$. Let $T =(k\beta)^{-1}$ be any characteristic temperature for the physical system. 
We define $\beta_\star = 1/kT_\star$ through the relation
\begin{equation}
	E_{\rm max} - E_s \eqqcolon kT_\star \ln 2,
\end{equation}
where $E_{\rm max} = \max\{E_0, E_1, E_2\}$ and $E_s$ is the eigenvalue of the eigenstate that overlaps the negative component of $\rho_S(\epsilon)$ in the Wigner representation. We define a threshold noise level,
\begin{equation}
...
We can now provide distillation bounds that depend on energetic and entropic aspects of the protocol. However, this is done relative to a reference temperature that can be chosen freely. The measures associated with this protocol are then determined by how it would disturb this reference equilibrium state $\sigma$ to some different state $\sigma'$. More precisely, the reference equilibrium state of the $n$ qutrits is given by
\begin{equation}
\sigma^{\otimes n} = \left ( \frac{e^{-\beta H}}{\Z} \right )  ^{\otimes n}.
\end{equation}
Since we are concerned with magic distillation we may assume that the reference state $\sigma$ is not a magic state, and so has non-negative Wigner distribution. Instead we shall assume that it is a full-rank stabilizer state for the qutrit. A given magic protocol on the $n$ qutrits will correspond to a quantum channel $\E$ acting on $\rho_S(\epsilon)^{\otimes n}$. Our analysis associates to this protocol energy/entropy measures by considering how $\E$ transforms this reference equilibrium state. For simplicity we assume that the action of $\E$ on $\sigma^{\otimes n}$ is also such that correlations can be neglected in the $n\rightarrow \infty$ limit, and so $\E(\sigma^{\otimes n}) = (\sigma')^{\otimes n}$ for some state $\sigma'$. This assumption simplifies the application of majorization, however we note that work has also been done on the case of where correlations build up and one only looks at the marginal state on an individual subsystem \ddd{[need to cite here]} and so a more detailed analysis can also be performed.

Finally, we can paramterise the state $\sigma'$ by defining a fiducial Hamiltonian $H'$, so that $\sigma' = e^{-\beta H'} / \Z$, while we denote all quantities relevant to this output state $\sigma'$ by priming them, leading to $E'_{\rm{max}}, E'_s, \beta'_\star$ and $\epsilon'_\star$.

\begin{theorem}\label{thm:free-energy}
	Consider a magic distillation protocol on $n \gg 1$ identical qutrits under the assumptions just stated.

Then the distillation rate $R$ of the protocol is bounded as follows:
\begin{equation}\label{eq:rate_bounds_proof}
	...
	\end{cases}
\end{equation}
In the above expressions $F$ is the free energy of the state $\sigma \coloneqq e^{-\beta H}/\Z$ and $F'$ is the free energy of the single qutrit state $\sigma' = e^{-\beta H'}/\Z'$ defined via the action of the protocol on the reference equilibrium state. \ddd{[Need strategy to incorporate conditions in simple, clear way.]}
\end{theorem}
 The state $\sigma'$ is generally a non-equilibrium state for the system, however one can still associate a free energy value to the output by considering subsequently changing the Hamiltonian $H \rightarrow H'$ such that equilibrium is restored at the reference temperature $T$. For example, suppose that $H=H'$ then this implies the protocol would leave the equilibrium state unchanged and so corresponds to a Gibbs-preserving map \ddd{[cite Philippe]}. In the more general case $\Delta F := F' - F \ne 0$ and we have a free energy difference between the initial and final times, with respect to the relevant Hamiltonians. This is similar to the free energy differences that occur in fluctuation relations, where the final state of the system can be arbitrarily far from equilibrium, yet a free energy term for the final system still arises.
