...
\begin{align}
\hspace{-1cm}\max_{k_1, k_2, \dots, k_{2n}} [ w_{k_1}w_{k_2}\cdots w_{k_{2n}}] &\le \max_{k_1, k_2, \dots, k_{2n}}[|w_{k_1}w_{k_2}|\cdots |w_{k_{2n-1}} w_{k_{2n}}|] \nonumber \\
...
Using the above results, we can show that $H_{2m}$ is Schur-concave for all quasi-distributions.
In particular, for any non-negative integer $m$, we have that
\begin{equation}
	W_{\rho_n}(\bmx) \succ W_{\rho'_{n'}}(\bmx) \mbox{ implies } H_{2m}(\rho_n) \leq H_{2m}(\rho'_{n'}).
\end{equation}
By additivity of $H_{2m}$, we get $n H_{2m}(\rho) \leq n' H_{2m}(\rho')$, or equivalently,
\begin{equation}
	\frac{n'}{n} \geq \frac{H_{2m}(\rho)}{H_{2m}(\rho')}.
\end{equation}

\nick{Wrong way? I don't think that $m \rightarrow \infty$ leads to our bound}
