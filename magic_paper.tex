...
\end{equation}
a threshold which is looser than the one provided by majorization for all $\epsilon > 0$. For example, for $\epsilon = 0.4$ we obtain $R\le 3.27 $ while the mana bound gives $R_{\rm{mana}} \le 1.89$. \ddd{[Am I making some mistake here?? Mana does better according to these numbers? Does it only do better for a region of $\epsilon$ values? Perhaps plot the functions together.]}
...
\end{equation}
so pre-order $\prec_{\gamma_\beta}$ is well defined. \ddd{[Not clear where this equation comes from.]}
...
\begin{equation}
	...
	\end{split}
\end{equation}
\ddd{[Definitely impressed by this Latexing...but perhaps just give the multiplicities and spectrum values. I.e. ``(blah) with multiplicity (blah) and ...'']} For the Strange state we have
\begin{equation}
	\begin{split}
