...
\begin{equation}\label{eq:ethresh}
	n_{\rm{th}}(\epsilon, \epsilon', \sigma) \coloneqq \min{\{n: \lc{\noisysn}{\sigma} \geq \lc{\noisys(\epsilon')}{\sigma}\}}.
\end{equation}}
...
\begin{align}
	...
	&\max\limits_{\substack{\alpha = 0,2, \\ \dots,n-2,n}}{\Big\{ (-\wn)^{n-\alpha} \wp^{\alpha} e^{\beta (n-\alpha)E_0} e^{\beta \alpha E_{\rm{max}}} \Big\}} = \nonumber\\
...
\begin{enumerate}
	\item $\rm{(C1)} \rightarrow \rm{(C1)}$ if $E_{\rm{max}} = E_0$ OR $E_{\rm{max}} > E_0$, $\beta < \beta_{\rm{th}}$ and $\epsilon' < \epsilon  \leq \epsilon_{\rm{th}}$.
	\item $\rm{(C2)} \rightarrow \rm{(C1)}$ if $E_{\rm{max}} > E_0$, $\beta < \beta_{\rm{th}}$ and $\epsilon' \leq \epsilon_{\rm{th}} < \epsilon$.
	\item $\rm{(C2)} \rightarrow \rm{(C2)}$ if $E_{\rm{max}} > E_0$, $\beta \geq \beta_{\rm{th}}$.
\end{enumerate}
Note that $\rm{(C1)} \rightarrow \rm{(C2)}$ is impossible because it would imply $\epsilon \leq \epsilon_{\rm{th}} \leq \epsilon' < \epsilon$, a contradiction.
...
This leads to an upper bound for the distillation rate $R=n'/n$ of pure Strange states ($\epsilon' = 0$),
\begin{equation}
	R \leq R(\beta) \coloneqq 1 - \frac{\ln{\left(\frac{1}{1-\frac{1}{3}\epsilon} \right)} + (E_{\rm{max}} - E_0)(\beta_{\rm{th}} - \beta)}{\beta (E_0 - F_\beta)},
\end{equation}
...
Consider pure Strange state distillation at high temperatures, $\beta \leq \beta_{\rm{th}}$.
...
At every $\beta > 0$, the rates indicate a transition at $\epsilon_{\rm{th}}$.
...
\nick{it holds for $0 \leq \epsilon \leq \epsilon_*$, where $\epsilon_\star$ is the noise level ($ \leq \epsilon_{\rm{th}}$) at which the state conversion turns from $\rm{(C1)} \rightarrow \rm{(C1)}$ to $\rm{(C2)} \rightarrow \rm{(C1)}$. Need to adjust~\cref{fig:distill_bounds_temp} accordingly}
    }
