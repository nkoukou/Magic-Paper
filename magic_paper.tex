...
\title{Thermodynamic structures for magic states in quantum computation}
...
Identifying magic as a resource for quantum universality has led to several theories which try to provide a framework for its quantification and manipulation \cite{cit:veitch2, cit:howard, Wang_2019, Seddon_2021, Okay_2021}.
The main question that such a theory attempts to answer is:
\begin{center}
    \emph{How do physical constraints, such as hardware limitations, noise bias or thermodynamics, restrict magic distillation protocols?}
\end{center}

Such constraints make stabilizer states inequivalent, and can give rise to stabilizer states that are invariant under the constraints. In this work we shall approach this question using tools from statistical mechanics and thermodynamics. In particular, we shall view \emph{magic as a form of free energy}, relative to the set of stabilizer states. This perspective takes the convex set of stabilizer states as a manifold of ``equilibrium'' states. However, while a free energy difference exists between two thermodynamic equilibrium states at temperatures $T_1$ and $T_2$, here we focus in on the degree to which the magic state lies off the set of stabilizer states, and therefore magic is more accurately viewed as a form of non-equilibrium free energy that is strictly non-classical.

Our approach relies on the discrete Wigner representation of quantum states, in which all states and operations can be described on a discrete phase space. Crucially, we focus on magic states with negativity in their Wigner function, which is known to be a necessary condition for universality in the state-injection model. However, following a thermodynamic perspective in this context raises a novel problem, namely it is impossible to define entropies for quasi-distributions. We circumvent this obstacle by making use of a more fundamental tool in thermodynamics -- majorization theory.

Our construction also provides distillation bounds without relying on magic monotones -- instead of projecting the pre-order of magic states onto the real line (which is a monotone) we instead cover the pre-order of states with a collection of simpler `fragments', which admit analysis through the lens of majorization. The choice of fragment breaks the equivalence of all stabilizer states in the theory and can be used to consider adding physical constraints in the form of accessible protocols. \nick{We need to give some concrete example here, we keep repeating this ``advantage'' abstractly}

We provide a range of upper bounds, which reveal that the thermodynamic structure in the presence of negativity is surprisingly non-trivial and displays a range of features that do not appear in classical statistical mechanics. We also discuss how this approach could be used to construct explicit lower bounds on distillation. This in turn raises non-trivial questions that are also of interest in statistical mechanics. More precisely: what are the conditions for majorization that respects the symplectic structure of phase space?

\subsection{Prior text that might be useful}
...
In fact, all states with non-negative Wigner distributions have been proven to be efficiently classically simulable~\cite{cit:mari}, a result that serves as a generalization of the Gottesman-Knill theorem.
The Wigner distribution of a state in odd prime dimensions arises as the unique quasi-probability representation of quantum theory that identifies non-contextuality exactly with the states that are efficiently classically simulable~\cite{Delfosse_2017, cit:howard2, cit:veitch2}.
...
The paper is structured as follows. In~\cref{sec:ps}, we formally define the Wigner representation of quantum states and outline its relevant properties. In~\cref{sec:frag}, we introduce majorisation and apply it on general magic theories. We go on to prove magic distillation bounds which account for abstract thermodynamic considerations in~\cref{sec:unital,sec:stab} and conclude in~\cref{sec:outlook}.
...
If we have a magic state $\rho$ that has negativity in its Wigner representation, then as discussed, its Lorenz curve over-shoots $1$ and reaches a non-trivial maximum $L_\star$ that depends on the particular state. In fact, we can establish the following general result.
