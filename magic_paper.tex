...
In~\cref{app:lcsu_technical}, we define explicit expressions for the Lorenz curve of the $n$-copy, $\epsilon$-noisy Strange state in the unital fragment, 
...
It is clear in~\cref{fig:lcs} that Lorenz curves are fully defined by the coordinates of their elbows.
For standard Lorenz curves, elbow coordinates are defined in terms of the two functions,
\begin{align}	
...
The exact expresions depend on the noise level $\epsilon$, the parity of the number of copies $\NN$ and whether the location of the elbow relative to the curve maximum (LHS vs RHS).
All cases are presented in~\cref{tab:lcsu_coord_elb}.
\begin{table}[h]
  ...
  \label{tab:lcsu_coord_elb}
\end{table}

The coordinates of any point on the standard Lorenz curve is given by

\begin{equation}
    \II(\ii,\jj,\kk) \coloneqq \kk + \left[ (\jj-1) + \sum_{\dummy=0}^{\ii-1} 8^{2\dummy}\binom{\tt}{2\dummy} \right]9^{\mm},
\end{equation}

\begin{align}
    \II(\ii,\jj,\kk=1) &= \jj + \sum_{\dummy=0}^{\ii-1} 8^{2\dummy}\binom{\nn}{2\dummy}, \\
    \II'(\ii',\jj',\kk') &= \kk' + \left[ (\jj'-1) + \sum_{\dummy=0}^{\ii'-1} 8^{2\dummy}\binom{\tt}{2\dummy} \right]9^{\mm}.
\end{align}
 
In particular, it can used in the discussion of standard Lorenz curves to set the necessary constraints,
\begin{align}
