...
Consider a general qutrit Hamiltonian 
\begin{equation}
	H = E_0 \ketbra{0} + E_1 \ketbra{1} + E_2 \ketbra{2},
\end{equation}
where the eigenbasis coincides with the computation basis of the Pauli Z operator.
The Wigner distribution of any qutrit thermal state $\gamma_\beta$ is calculated directly by the definition in~\cref{eq:wstate},
\begin{equation}
	\W[\bmx]{\gamma_\beta} = \frac{e^{-\beta E_{x_0}}}{d\Z_\beta}\ \text{for all}\ \bmx \in \pd,
\end{equation}
and always consists of strictly positive components, therefore the pre-order $\prec_{\gamma_\beta}$ is well defined.

Consider the simple scenario, where $H$ has the spectrum 
\begin{equation}
	(E_0, E_1, E_2) = (1,0,0).
\end{equation}
...
\begin{table}[h]
  ...
  \begin{tabular}{c|c|c|c}
    $\bmx = (x_0, x_1)$ & $W_{\noisys}(\bmx)$ & $W_{\gamma_\beta}(\bmx)$ & $W_{\noisys(\epsilon)}(\bmx)/W_{\gamma_\beta}(\bmx)$ \\
    \hline
    \makecell{$(0,0)$ \\ \vdots \\ $(0,2)$} & $\displaystyle \frac{1}{6} - \frac{1}{18}\epsilon$ & $\displaystyle \frac{1}{3(1+2e^\beta)}$ & $\displaystyle \left(\frac{1}{2}-\frac{1}{6}\epsilon \right)(1+2e^\beta)$ \\
    \hline
    \makecell{$(1,0)$ \\ $\vdots$ \\ $(2,1)$} & $\displaystyle \frac{1}{6} - \frac{1}{18}\epsilon$ & $\displaystyle \frac{1}{3(2+e^{ -\beta})}$ & $\displaystyle \left(\frac{1}{2}-\frac{1}{6}\epsilon \right)(2+e^{-\beta})$ \\
    \hline
    $(2,2)$ & $\displaystyle -\frac{1}{3} + \frac{4}{9}\epsilon$ & $\displaystyle \frac{1}{3(2+e^{-\beta})}$ & $\displaystyle \left(-1 + \frac{4}{3}\epsilon \right)(2+e^{-\beta})$ \\
    \hline
  \end{tabular}
  \caption{Wigner components $W_{\gamma_\beta}(\bmx)$, $W_{\noisys}(\bmx)$ and the associated slope $W_{\noisys}(\bmx)/W_{\gamma_\beta}(\bmx)$ listed in order of decreasing slope (holds for all $\beta$).}
  \label{tab:thermal_w}
\end{table}

To construct the 1-copy Lorenz curve $\lc{\noisys(\epsilon)}{\gamma_\beta}$, we simply take the cumulative sum of the Wigner components in order of decreasing slope $W_{\noisys}(\bmx)/W_{\gamma_\beta}(\bmx)$.
The first elbow of the 1-copy Lorenz curve is
\begin{equation}
	(x_0, L_0) = \left( \frac{1}{1+2e^\beta}, \frac{1}{2} - \frac{1}{6}\epsilon \right).
\end{equation}
