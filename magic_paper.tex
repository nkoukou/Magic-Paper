...
\title{General thermodynamic constraints on magic states for quantum computation}
...
The past few years have seen rapid progress towards the goal of a fault-tolerant quantum computer\ddd{[CITE]}. However many challenges remain and there is increasing need for theory to take into account physical limitations of the hardware involved. The surface code~\cite{cit:bravyi}\ddd{[CITE MORE]} is a leading framework for fault-tolerance with very high error thresholds. model for quantum computation is a central approach to realising a full-scale, fault-tolerant quantum computer. Within this scheme, Clifford unitaries can be implemented in a robust, fault-tolerant way. However, due to the Eastin-Knill theorem~\cite{Eastin_2009}, we also know that it is impossible to have a universal set of transversal gates, and although Clifford unitaries can be realised transversally\ddd{[CSS code for this]}, one needs to find ways around the Eastin-Knill restriction. This can be achieved by injecting in quantum states, called magic states, which promote the Clifford group to universal quantum computing. However, the obstacle to this is that these states are invariably noisy and so protocols involving stabilizer operations must be employed to purify many copies of the magic states and improve the overall performance of the induced quantum gates. A central question is then the overhead on purifying many copies of a magic state into less noisy forms. 

To address such questions, concrete distillation protocols have been developed, such as the Bravyi-Haah protocol that provides a quadratic reduction in noise per cycle. Such distillation rates have been improved in more recent works \ddd{[CITE]}. There is also analysis of magic protocols from the perspective of magic monotones, which provide upper bounds on distillation rates and find application in the analysis of gate synthesis \ddd{[CITE Earl+Mark paper, plus other synthesis paper(s)]}. These frameworks for magic view magic states as resource states with respect to a natural class of quantum operations that are consider cheap, or free, such as stabilizer operations~\cite{Gour_2019, cit:ahmadi, cit:seddon, Wang_2019}.
  
  
Recent work towards fault-tolerance has begun to bridge the gap between abstract theory and experiment. Extensive work has been done on error mitigation \ddd{[cite]}, and the incorporation of hardware physics into the theoretical modes. For example, the XZZX code\ddd{[cite]} is a variant of the surface code that incorporates noise bias explicitly and has been shown to attain the hashing bound of random codes. 

In this work we develop a framework to analyse magic state distillation protocols where explicit physical constraints are manifest. These physical constraints are encoded in the allowed fixed-point structure of the protocol, for example a minimal temperature attainable in the device, noise-biases or fixed-point structure associated with restricted gate-sets. The approach we take is most closely aligned with resource theories of magic, however our approach differs in two ways. Firstly, we obtain distillation upper bounds without the use of monotones, and secondly our distillation bounds are functions of the physical constraints and so not directly comparable to global magic monotones.

 
We approach this problem using tools from statistical mechanics and recent work from the overlap of thermodynamics and quantum information theory\ddd{[cite]}. In particular, we shall view \emph{magic as a form of free energy}, relative to the set of stabilizer states. This perspective takes the convex set of stabilizer states as the set of ``equilibrium'' states, and magic corresponds to a form of non-equilibrium free energy that is strictly non-classical.

Our analysis relies on the discrete Wigner representation of quantum states, in which all states and operations can be described on a discrete phase space \ddd{[cite]}. Crucially, we focus on magic states with negativity in their Wigner representation, which is known to be a necessary condition for universality in the state-injection model. Interestingly, taking a thermodynamic perspective in this context raises a problem, namely it is impossible to define entropies for quasi-distributions. We circumvent this obstacle by making use of a more fundamental tool in thermodynamics -- majorization theory \ddd{[cite]}. To our knowledge majorization of quasi-distributions has not been considered in quantum physics before, and therefore our analysis is of broader interest to the study of non-classicality in quantum systems.

Our construction provides distillation bounds without relying on magic monotones. Instead of projecting the pre-order of magic states onto the real line (which is what a monotone involves) we instead cover the pre-order of states with a collection of simpler `fragment' pre-orders that admit analysis in terms of majorization. It is this choice of fragment that breaks the equivalence of all stabilizer states in the theory and can be used to consider adding physical constraints in the form of accessible protocols. \nick{We need to give some concrete example here, we keep repeating this ``advantage'' abstractly}
...
Finally, we discuss how our approach could be improved, and also used to construct explicit lower bounds on distillation that take into account physical structure. Our work also raises non-trivial questions that are also of interest in statistical mechanics. Specifically: what are the majorization conditions that respect the symplectic structure of phase space?
...
In this section we give some generic aspects of Lorenz curves for magic states. In particular these allow us to interpret previous magic monotones in a new light. The first result provides a extremely simple way to see that the sum-negativity of a magic state is a monotone.

\begin{lemma} Relative majorization in any fragment implies the monotonicity of sum-negativity $sn(\rho) := \frac{1}{2} (\sum_{\x} |W_\rho(\x) | - 1)$ of a magic state $\rho$. 
\end{lemma}
\begin{proof}
 We make use of a variant form of relative majorization \ddd{[cite]}, which says that $\p \succ_{\r} \q$ if and only if
	\begin{equation}
\sum_k | p_k - t r_k | \ge \sum_k |q_k - t r_k|,
\end{equation}
for all $t\in \mathbb{R}$. If we choose $t=0$ then we get the single condition that $\sum_k |p_k| \ge |\sum_k |q_k|$, independent of the choice of $\r$. Applying this to the Wigner quasi-distribution of a quantum state immediately gives the result.
\end{proof}
If we have a magic state $\rho$ that has negativity in its Wigner representation, then as discussed, its Lorenz curve over-shoots $1$ and reaches a non-trivial maximum $L_\star$ that depends on the particular state. The relation with sum-negativity and mana is provided by the following result.
...
The first elbow constraint involves a simple interpolation, done in~\cref{app:elb_constraints}, in order to find the coordinates of the output Lorenz curve point which is vertically aligned with the first elbow of the input Lorenz curve. 
...
\item As a consequence of the actual hardware limitations, noise bias or ambient thermal environment reflected by the invariant state $\sigma$ for the system.
\end{enumerate}
...
\begin{equation}\label{eq:first_elb_coords}
	...
	\end{cases}
\end{equation}
To obtain the distillation bounds from majorization we can now compute the above terms for both the input and output states. Depending on the noise parameters $\epsilon, \epsilon'$ for the initial and target states the Lorenz curve data may fall into either case $\rm{(C1)}$ or case $\rm{(C2)}$.
...
\end{proof}
We note that the above bounds are not optimal, and since we only use part of the Lorenz curve they can be improved upon. However, we find numerically that they do better than, for example, the constraint coming from the peak of the curve. Similar bounds can be obtained from the decreasing part of the Lorenz curve, but they do not shed any more light on distillation bounds.
...
\end{equation}
The condition for this that $L_{\rho |\sigma } (x) \ge L_{\rho'|\sigma'} (x)$, and so exactly the same analysis applies as in the previous case, but now with the Gibbs-recaling parameters for the output state being different from the input state \ddd{[cite recent Renes paper]}.

If the physical system has Hamiltonian $H$, and experimentally it can be cooled down to some reference temperature $T$ then we can consider the reference equilibrium state $\sigma = e^{-\beta H}/\Z$, and estimate the energetic and entropic of protocols in terms of how they transform $\sigma$ to some other $\sigma'$. For example, if $\sigma'$ is close to the ground state of the system then we know that a great deal of entropy is dumped by the process. This allows us to parameterise our distillation bounds in more convenient terms.

The state $\sigma'$ will in general not be an equilibrium state, or have any temperature associated to it. However we can still associate thermodynamic variables to it as follows: we consider what changes in Hamiltonian are required in order for the output state $\sigma'$ to still be an equilibrium state at the reference temperature $T$. Therefore $H\rightarrow H'$ such that $\sigma' = e^{-\beta H'}/\Z'$ and so the Lorenz curve of the output state $\rho'$ relative to $\sigma'$ will be expressible in terms of the free energy $F'(\beta) = -\beta^{-1} \log \Z'$ and the spectrum of the Hamiltonian $H'$. We find that \ddd{[Details? Expect that we now get free energy differences $\Delta F := F' - F$ appearing.]}
