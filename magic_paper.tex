...
\begin{equation}
	H_\alpha(\rho) := \frac{1}{1-\alpha} \log_2 \sum_{\bmx} W_{\rho}(\bmx)^\alpha,
\end{equation}
...
\end{proof}
\nick{Combining ALL the properties above (and nothing more) leads to the general bound}
...
\begin{proof} If $H_\alpha (W_\rho(\x)) <0 $ then this implies that $\log \sum_{\x} W_\rho (\x)^\alpha > 1$, and it is known that $H_\alpha$ is always non-negative on probability distributions, so $W_\rho(\x)$ must be a strict quasi-distribution with negativity.
...
\end{equation}
\nick{In this step, we also need to show that $H_\alpha$ is increasing in $\alpha$}
...
\ddd{[Looking at this limit it almost feels like the $\alpha \rightarrow 1$ limit is actually giving mana....modulo the technical problem of taking the limit without calculus. I will think more about this point.]}
