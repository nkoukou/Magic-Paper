...
\begin{equation}\label{eq:ethresh}
	\begin{split}
	\epsilon_{\star}(n, \tt, \epsilon', \sigma) \coloneqq &\sup{\{\epsilon:\ \lc{\rho(\epsilon)\too{n}}{\sigma}}(x) \geq \\
	&{\lc{\rho(\epsilon')\too{\tt}}{\sigma}(x),\ x\in[0,1]\}}.
	\end{split}
\end{equation}
The MSD process~(\ref{eq:dist}) is impossible for all $\epsilon > \epsilon_{\star}(n, \tt, \epsilon', \sigma)$.

\nick{Can also define a distillation bound
\begin{equation}\label{eq:ethresh}
	n_{\star}(\epsilon, \epsilon', \sigma) \coloneqq \min{\{n: \lc{\noisysn}{\sigma} \geq \lc{\noisys(\epsilon')}{\sigma}\}}.
\end{equation}}
...
\end{align}
where we use a three-index parametrisation for the coordinates, with $\ii$ the index of the elbow that follows, $\jj = 1,\dots,\mlt_\ii$ and $\kk=1,\dots,9^{2(n-\tt)}$.
Therefore, for the initial state $\noisys\too{2n}$ we have
\begin{equation}
	p_{\ii\jj\kk} = \frac{\jj}{8^{2\ii}\binom{2n}{2\ii}},
\end{equation}
with $\ii=0,\dots,n,\ \jj(\ii) = 1,\dots,8^{2\ii}\binom{2n}{2\ii}$ and $\kk = 1$.
For the target state $\noisys(\epsilon')\too{2\tt} \otimes \left( \frac{1}{3}\id \right)\too{2(n-\tt)}$,
\begin{equation}
	p'_{\ii'\jj'\kk'} = \frac{\kk' + (\jj'-1)9^{2(n-\tt)}}{9^{2(n-\tt)} 8^{2\ii'}\binom{2\tt}{2\ii'}}
\end{equation}
with $\ii'=0,\dots,\tt,\ \jj'(\ii') = 1,\dots,8^{2\ii'}\binom{\tt}{2\ii'}$ and $\kk' = 1,\dots,9^{2(n-\tt)}$.
\fi
...
\begin{align}
	...
	\bmw(\noisys) &\coloneqq (-v, u, u, u) \\
	\bmw(\gamma) &\coloneqq \frac{1}{3\Z} \left( e^{-\beta E_0}, e^{-\beta E_0}, e^{-\beta E_1}, e^{-\beta E_2} \right) \\
	\bmw(\noisys | \gamma) &\coloneqq 3\Z \left( -v e^{\beta E_0}, u e^{\beta E_0}, u e^{\beta E_1}, u e^{\beta E_2} \right) \label{eq:bmw_rescaled}
\end{align}
...
\begin{align}
	w(\noisys)_{ijk} &= (-v)^{n-\alpha} u^{\alpha} \\
	w(\gamma)_{ijk} &= (3\Z)^{-n} e^{-\beta (n-\alpha)E_0} e^{-\beta ( i E_0 + j E_1 + k E_2 )} \\
	w(\noisys | \gamma)_{ijk} &= (3\Z)^{n} (-v)^{n-\alpha} u^{\alpha} e^{\beta (n-\alpha)E_0} e^{\beta ( i E_0 + j E_1 + k E_2 )}
\end{align}
...
Notice that for $0 \leq \epsilon \leq 3/7$, we have $v \geq u$. We assume that $n$ is even, ans so we then need the sum $\alpha = i+j+k$ to be even, so that the expression is positive.
...
%Hence, we have \ddd{Why did the $\Z$ term disappear?}
%\begin{align}
%	\bmw(\noisys | \gamma)_{\rm{max}}
%&=\max\limits_{\substack{\alpha = 0,2, \\ \dots,n-2,n}}{\Big\{ v^{n-\alpha} u^{\alpha} e^{\beta (n-\alpha)E_0} e^{\beta \alpha E_{\rm{max}}} \Big\}}  \nonumber\\
%	&= v^n e^{n\beta E_0}\max\limits_{\substack{\alpha = 0,2, \\ \dots,n-2,n}}{\Big\{ \left( \frac{u}{v} e^{\beta (E_{\rm{max}} - E_0)} \right)^{\alpha} \Big\}}
%\end{align}
%If the expression $\frac{u}{v} e^{\beta (E_{\rm{max}} - E_0)}$ is less than $1$ then the maximum occurs at $\alpha=0$, and otherwise the maximum occurs at $\alpha = n$.
%The threshold value of $1$ is achieved for this term when the noise level $\epsilon$ is equal to
%\begin{equation}
%	\epsilon_{\star}(\beta) := 3 - \dfrac{18}{8-e^{(E_{\rm{max}} - E_0)\beta}}.
%	%\beta \leq \frac{1}{E_{\rm{max}} - E_0}\ln{\left( 8-\frac{18}{3-\epsilon} \right)}
%\end{equation}
%For the case $E_{\rm{max}} = E_0$ this threshold is constant in temperature and given by Since $ E_{\rm max} - E_0 \ge 0$ the threshold $\epsilon_{\star}$ is a decreasing function of $\beta$, and it achieves its maximum value $3/7$ for $(E_{\rm{max}} - E_0)\beta = 0$, and a value equal to $0$ for
%\begin{equation}
%	\beta_{\star} \coloneqq \frac{1}{E_{\rm{max}} - E_0} \ln2.
%\end{equation}
%For $\beta > \beta_{\star}$, $\epsilon_{\star}$ is negative, meaning that the noise level $\epsilon$ will always be higher than the threshold.
...
%\end{equation}
%
%The maximum slope \ddd{``maximum slope??''} finally is
%\begin{equation}
%\bmw(\noisys | \gamma)_{\rm{max}} =
%	\begin{cases}
%		v^n e^{n\beta E_0}, &\epsilon \leq \epsilon_{\star},\ \hspace{30pt}\rm{(C1)}	\\
%		u^n e^{n\beta E_{\rm{max}}}, &\epsilon > \epsilon_{\star}.\ \hspace{32pt}\rm{(C2)} 
%	\end{cases}
%\end{equation}
...
%	\item $0 < \beta < \beta_{\star}$. Then, $0 < \epsilon_{\star} < 3/7$ and $\rm{(C1)}$ holds for $\epsilon \leq \epsilon_{\star}$, while $\rm{(C2)}$ holds for $\epsilon \geq \epsilon_{\star}$.
%	\item $\beta \geq \beta_{\star}$. Then, $ \epsilon_{\star} \leq 0$ and $\rm{(C2)}$ holds for all $\epsilon$.
%\end{itemize}

In $\rm{(C1)}$, the multiplicity is $m_{000} = 1$ and the corresponding Wigner components are $\bmw(\noisys)_{000}, \bmw(\gamma)_{000}$.
In $\rm{(C2)}$, we have $E_{\rm{max}} = E_1$ ($E_{\rm{max}} = E_2$), the multiplicity is $m_{0n0} = 3^n$ ($m_{00n} = 3^n$) and the corresponding Wigner components are $\bmw(\noisys)_{0n0}, \bmw(\gamma)_{0n0}$ ($\bmw(\noisys)_{00n}, \bmw(\gamma)_{00n}$).
...
\end{enumerate}
Note that $\rm{(C1)} \rightarrow \rm{(C2)}$ is impossible because it would imply $\epsilon \leq \epsilon_{\star} \leq \epsilon' < \epsilon$, a contradiction.
...
\begin{enumerate}
	...
	\begin{align*}
		x_0 &= \left(\dfrac{e^{-\beta E_0}}{3\Z_\beta}\right)^n = \left(\dfrac{e^{-\beta E_0}}{3\Z_\beta}\right)^{n-\tt} \left(\dfrac{e^{-\beta E_0}}{3\Z_\beta}\right)^{\tt} \\
		&< \left(\dfrac{e^{-\beta E_0}}{3\Z_\beta}\right)^{\tt} = x_0'		
	\end{align*}
	\item $\rm{(C2)} \rightarrow \rm{(C1)}$.
	It is always true that
	\begin{align*}
		&\Z_\beta^{n-\tt} > (e^{-\beta E_{\rm{max}}})^{n-\tt} = e^{-\beta (n-\tt) E_{\rm{max}}}, \text{ so} \\
		x_0 &= \left(\dfrac{e^{-\beta E_{\rm{max}}}}{\Z_\beta}\right)^n = \dfrac{e^{-\beta(n E_{\rm{max}} - \tt E_0)}}{\Z_\beta^{n-\tt}} \left(\dfrac{e^{-\beta E_0}}{3\Z_\beta}\right)^{\tt} \\
		&< \dfrac{e^{-\beta(n E_{\rm{max}} - \tt E_0)}}{e^{-\beta (n-\tt) E_{\rm{max}}}} \left(\dfrac{e^{-\beta E_0}}{3\Z_\beta}\right)^{\tt} \\
		&= e^{-\beta \tt (E_{\rm{max}} - E_0)} \left(\dfrac{e^{-\beta E_0}}{3\Z_\beta}\right)^{\tt} \\
		&< \left(\dfrac{e^{-\beta E_0}}{3\Z_\beta}\right)^{\tt} = x_0'.
	\end{align*}
	\item $\rm{(C2)} \rightarrow \rm{(C2)}$.
	It is always true that $e^{-\beta E_{\rm{max}}} / \Z_\beta \leq 1$, so
	\begin{align*}
		x_0 &= \left(\dfrac{e^{-\beta E_{\rm{max}}}}{\Z_\beta}\right)^n = \left(\dfrac{e^{-\beta E_{\rm{max}}}}{\Z_\beta}\right)^{n-\tt} \left(\dfrac{e^{-\beta E_{\rm{max}}}}{\Z_\beta}\right)^{\tt} \\
		&\leq \left(\dfrac{e^{-\beta E_{\rm{max}}}}{\Z_\beta}\right)^{\tt} = x_0'	
	\end{align*}
\end{enumerate}
...
\begin{align}
	...
	\epsilon &\leq \frac{3}{4} - \frac{3}{4} \left( 1 - \frac{4}{3}\epsilon' \right)^{\frac{\tt}{n}} \left( \frac{e^{-\beta E_0}}{\Z_\beta} \right)^{1 - \frac{\tt}{n}}	
\end{align}
...
\begin{align}
	...
	\epsilon &\leq 3 - 6 \left( 1 - \frac{4}{3}\epsilon' \right)^{\frac{\tt}{n}} \frac{e^{-\beta E_{\rm{max}}} e^{\frac{\tt}{n}\beta E_0}}{\Z_\beta^{1-\frac{\tt}{n}}}	
\end{align}
This leads to an upper bound for the distillation rate $R=n'/n$ of pure Strange states ($\epsilon' = 0$),
\begin{equation}
...
\begin{align}
	...
	\epsilon &\leq 3 - 6 \left( \frac{1}{2} - \frac{1}{6}\epsilon' \right)^{\frac{\tt}{n}} \left( \frac{e^{-\beta E_{\rm{max}}}}{\Z_\beta} \right)^{1 - \frac{\tt}{n}}	
\end{align}
This leads to an upper bound for the distillation rate $R=n'/n$ of pure Strange states ($\epsilon' = 0$),
\begin{equation}
...
\nick{it holds for $0 \leq \epsilon \leq \epsilon_*$, where $\epsilon_\star$ is the noise level ($ \leq \epsilon_{\star}$) at which the state conversion turns from $\rm{(C1)} \rightarrow \rm{(C1)}$ to $\rm{(C2)} \rightarrow \rm{(C1)}$. Need to adjust~\cref{fig:distill_bounds_temp} accordingly}
    }
...
\begin{align}\label{eq:g1}
	G_1(\noisys) &\coloneqq -v + 2u X + 3u Y + 3u Z \\
	\begin{split}
	...
	\\
	\begin{split}
	G_1(\noisys|\gamma) &\coloneqq (3\Z) \big( -v e^{\beta E_0} + 2u e^{\beta E_0} X \\
	&\hspace{41pt} + 3u e^{\beta E_1} Y + 3u e^{\beta E_2} Z \big)
	\end{split}
\end{align}
...
For $\beta = 0$, $\epsilon_{\star} = 3/7$, so $\epsilon \leq \epsilon_{\star}$ always.
In this case, $\widetilde{W}_{\rm{max}} = v^n e^{n\beta E_0}$ occuring in the region labelled by $(i,j,k) = (0,0,0)$.

For 	
\begin{equation}
	\beta \geq \beta_{\star} \coloneqq \frac{1}{E_{\rm{max}} - E_0} \ln2,
\end{equation}
$\epsilon_{\star} \leq 0$, so $\epsilon \geq \epsilon_{\star}$ always.

For $0 < \beta < \beta_{\star}$, the order between energy eigenvalues matters.
\begin{itemize}
\item If $E_{\rm{max}} = E_0$, only the first case is valid, since the noise constraint becomes $\epsilon \leq 3/7$.
In this case, $\widetilde{W}_{\rm{max}} = v^n e^{n\beta E_0}$ occuring in the region labelled by $(i,j,k) = (0,0,0)$.
\item If $E_{\rm{max}} = E_1$, then for low noise levels $\epsilon \leq \epsilon_{\star}$, $\widetilde{W}_{\rm{max}}  = v^n e^{n\beta E_0}$ occuring in the region labelled by $(i,j,k) = (0,0,0)$ and for high noise levels $\epsilon > \epsilon_{\star}$, $\widetilde{W}_{\rm{max}}  = u^n e^{n\beta E_1}$ occuring in the region labelled by $(i,j,k) = (0,n,0)$.
\item If $E_{\rm{max}} = E_2$, then for low noise levels $\epsilon \leq \epsilon_{\star}$, $\widetilde{W}_{\rm{max}}  = v^n e^{n\beta E_0}$ occuring in the region labelled by $(i,j,k) = (0,0,0)$ and for high noise levels $\epsilon > \epsilon_{\star}$, $\widetilde{W}_{\rm{max}}  = u^n e^{n\beta E_2}$ occuring in the region labelled by $(i,j,k) = (0,0,n)$.
\end{itemize}
