...
Notably, and in contrast with the first elbow, mana and max--thauma bounds, $R_{\rm{opt}}$ depends on the copies $m,n$, in the sense that it doesn't bound their ratio $m/n$, but it is specific to the process.
\begin{figure}[t]
    \centering
    ...
    Numerical bounds $R_{\rm{opt}}$ obtained by full Lorenz curve comparison for some processes with even number of copies up to 30 are also plotted illustrating that the first elbow constraint can be improved.
    }
