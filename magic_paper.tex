...
\begin{figure}[t]
	...
	Constraints on the efficiency of the protocol can be imposed by considering its action on a separate probe state $\tau$ which tranforms to $\tau'$ with free energy $F \rightarrow F'$.
	A ``magic free energy'' $\phi \rightarrow \phi'$ connects the two state conversions under the protocol. \nick{FIX}
    }
    \label{fig:sketch}
\end{figure}
...
\begin{proof}
	...
	By construction, the slope of the curve attains the values 
\begin{equation}
	\frac{\sum_{i=0}^k p_{\pi(i)} - \sum_{i=0}^{k-1} p_{\pi(i)}}{\sum_{i=0}^k r_{\pi(i)} - \sum_{i=0}^{k-1} r_{\pi(i)}} = \frac{p_{\pi(k)}}{r_{\pi(k)}},\ \text{with } k \geq 1,
\end{equation}
throughout the interval $[0,1]$, which is a non-increasing sequence due to the definition of $\pi$.
	Therefore, the Lorenz curve is concave.
\end{proof}
}
...
& \le L_{\p|\r}(w)
\end{align}
\nick{second inequality isn't clear to me}
...
& \le L_{\p|\r}(w)
\end{align}
\nick{again second inequality isn't clear to me}
...
\nick{As a result, every Strange state Lorenz curve in the unital fragment contains $n$ elbows. In the case of accidental degeneracies among Wigner components, some of these elbows may be straightened out.}
...
\hline 
  ...
  \nick{Rows labelled as LHS express the coordinates $x_i$ and then $L_i$, while rows labelled as RHS, express the differences $x_i - x_{\lfloor n/2 \rfloor}$ and then $L_i - L_{\lfloor n/2 \rfloor}$}.
...
\end{equation}
\nick{We stress that $3v(\epsilon')e^{-\beta (F' - \phi')} > 1$ always, because $\rho_{\rm{S}}(\epsilon')$ is not a free state, so the initial slope of its Lorenz curve exceeds $1$.}
