...
\end{equation}
where the eigenbasis coincides with the computational ($Z$) basis.
...
\begin{align}
	...
	&= \sum\limits_{k=0}^2 \frac{e^{-\beta E_k}}{\Z} \delta_{x_0, k} = \frac{e^{-\beta E_{x_0}}}{d\Z},
\end{align}
for all $\bmx \in \pd$. 
...
\end{equation}
Then, $Z = 2 + e^{-\beta}$.
Furthermore, for clarity, let $\chi \coloneqq \frac{1}{6}-\frac{1}{18}\epsilon$ be the positive and $-\psi \coloneqq -\frac{1}{3} + \frac{4}{9}\epsilon$ the negative Wigner components of $\noisys(\epsilon)$.
...
\begin{table}[h]
  ...
  \begin{tabular}{c|c|c|c}
    ...
    \hline
    $(2,2)$ & $\displaystyle -\psi$ & $\displaystyle \frac{1}{3Z}$ & $\displaystyle 3Z (-\psi)$ \\
    \hline
  \end{tabular}
...
\begin{equation}
	\tilde{\bmw\too{n}} = \frac{W_{\noisys\too{n}}(\bmx)}{W_{\gamma\too{n}}(\bmx)} = \left( \frac{W_{\noisys}(\bmx)}{W_{\gamma}(\bmx)} \right)\too{n} = \tilde{\bmw}\too{n}.
\end{equation}

According to~\cref{def:lc}, to construct the $n$-copy Lorenz curve, we need to sort $\tilde{\bmw\too{n}}$ in decreasing order and take the cumulative sum of the corresponding components $W_{\noisys\too{n}}(\bmx)$ and $W_{\gamma\too{n}}(\bmx)$.
