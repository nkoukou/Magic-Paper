...
\end{equation}
and therefore taking logarithms we have that $D_\infty (\rho^{\otimes 2n}, \tau^{\otimes 2n}) = n D_\infty(\rho^{\otimes 2} ,\tau^{\otimes 2})$ as required. \ddd{[Check details for any fine print.]}
...
\ddd{[TO-DO AND TIDY AND CHECK]} Okay great...this looks easy to do, we just need to compute the terms
\begin{equation}
...
R \le \frac{H_{2m}(\rho)}{H_{2m}(\rho')}.
\end{equation}
\ddd{[Plan: check all these calculations! Give the generalised unital bound. Then we just conjecture this extends to Renyi-divergences and leave it at this.]}
