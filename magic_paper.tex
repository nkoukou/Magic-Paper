...
\begin{figure}[t]
    ...
    \caption{\textbf{Distillation bounds in the unital fragment.} Distillation bounds obtained by the first elbow majorisation constraint $R_{\infty}$, the 10-Renyi bound $R_{10}$,o mana $\M$ and max--thauma $\theta_{\rm{max}}$ are plotted for $\epsilon' = 0$, up to error $\epsilon = 3/7$.
...
\begin{align}
	...
	&\ddd{= \frac{\alpha}{\alpha-1}\log 18+ \frac{1}{1-\alpha}\log \left[ 8(3-\epsilon)^\alpha + (6-8\epsilon)^\alpha \right]}
\end{align}
