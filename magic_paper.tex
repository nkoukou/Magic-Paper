...
Since the state $\frac{1}{3} \id$ is free, the interconversion problem is unaffected if we tensor in copies of it. 
...
The exact expressions for the coordinates of these points can take $2^3 = 8$ different forms, depending on whether the noise level $\epsilon$ is low/high (relative to $\frac{3}{7}$), the parity of the number of copies $\nn$ is even/odd and the location relative to the curve peak is on the left hand side (LHS -- including the maximum) / right hand side (RHS).
...
However, in principle we can pick any location $x_\star$ at which the Lorenz curves are defined and we then get a necessary distillation constraint, 
\begin{equation}
...
The first elbow of the target state is located at $x'_0 = \frac{1}{9^{\tt}} < x_\star$, so in order to find the target state Lorenz curve coordinate $L'_\star$, we can always interpolate between the origin and the first target state elbow, so that
\begin{equation}
	L'_\star = \frac{x_\star}{x'_0}L_0 = \frac{1}{9^{\nn-\tt}}\Phi_+\left(0;\tt,\frac{12-4\epsilon}{15-8\epsilon}\right) = \frac{(3-4\epsilon)^{\tt}}{9^{\nn}}.
\end{equation}
The process in~\cref{eq:sudist} is therefore possible if $L_\star \geq L'_\star$ or equivalently,
\begin{align}
	\frac{\nn}{\tt} &\geq \frac{\ln{(3-4\epsilon')}}{\ln{(3-4\epsilon)}}, \\
...
\nick{All numerical examples I have tested, for various different cases, are quite close}
