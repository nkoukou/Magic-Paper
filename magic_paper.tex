...
We prove the following result connecting simple majorization to relative majorization.
...
\subsubsection{Classical $\alpha$-Renyi entropy and divergence for quasi-distributions}
...
\end{equation}
where $\alpha$ can take values of the form $2a / (2b+1)$, for non-negative integers $a,b$ to ensure that the argument of the logarithm is positive.
\nick{Notation clashes with quantum Renyi divergence}
...
\begin{proof}
	...
	\begin{align}
		...
		&= \frac{1}{1-\alpha} \log_2 K^{1-\alpha} \sum_{i=1}^N w_i^{\alpha} \nonumber\\
...
\begin{theorem}\label{thm:Da_props} 
	Let $\tau$ be a state in the interior of $\F$ and $\alpha = 2a / (2b+1)$ for non-negative integers $a,b$
...
1. \nick{looks true numerically, maybe can use~\cref{thm:H2D}}

2. Assume $D_\alpha (\rho,\tau) = 0$.
Then,
\begin{equation}
	\sum_{\bmx} W_{\tau}(\bmx) \left( \frac{W_\rho(\x)}{W_\tau(\x)} \right)^\alpha = 1,
\end{equation}
\nick{so...}.
The converse holds by inspection.
...
4. \nick{This must hold due to~\cref{thm:H2D}}
\end{proof}
...
\ddd{Therefore if we have a Schur-Concave function $f(\x)$ for majorization of quasi-distributions then we can define a monotonic function for relative majorization via composition with $\Gamma_{\mathbf{a}}$ and its inverse.
...
\end{equation}}
\nick{I don't understand where the free energies came from. Are they known to be Schur-concave?}
...
\begin{theorem}
	...
	We have the following family of upper bounds on the distillation rate $R := n'/n$,
	\begin{equation}
		R \leq \frac{D_{\alpha}(\rho||\tau)}{D_{\alpha}(\rho'||\tau')},
	\end{equation}
	where $\alpha = \frac{2a}{2b-1}$ for any positive integers $a,b$ with $a \geq b$.
\end{theorem}
\begin{proof}
...
\begin{equation}
	K - nD_{\alpha}(W_{\rho} || W_{\tau_n}) \leq K - n'D_{\alpha}(W_{\rho'} || W_{\tau'_{n'}}),
\end{equation}
and then it is clear that we can rearrange it to
\begin{equation}
	\frac{n'}{n} \leq \frac{D_{\alpha}(\rho||\tau)}{D_{\alpha}(\rho'||\tau')}.
\end{equation}
\end{proof}

\ddd{The following is a nice thing to establish.}
...
\ddd{[Looking at this limit it almost feels like the $\alpha \rightarrow 0$ limit is actually giving mana....modulo the technical problem of taking the limit without calculus. I will think more about this point.]}
