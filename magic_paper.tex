...
% Proofreading --- Citations (Aberg -> {\AA}berg)
% Proofreading --- Main text (Spacing)
...
We note that this mapping to stochastic maps is not one-to-one. Not all stochastic maps on the phase space correspond to valid quantum operations. The reason is that the maps must also respect the symplectic structure of the phase space, which is an additional non-trivial constraint.
...
\end{proof}
While we have integers $a,b$ such that $0 < \alpha=\frac{2a}{2b-1} < 1$ and $H_\alpha(\w)$ is well-defined, it turns out that monotonicity does not hold if we drop the condition $a \ge b$. If $\alpha =2a/(2b-1)$ with $\alpha < 1$, then $g(w) \coloneqq \frac{-w^{\alpha-1}}{\alpha-1}$ is no longer monotonic for all $w \in \mathbb{R}$, and the problem occurs when comparing $w_i < 0$ and $w_j >0$. As an analogy, consider the function $g(x) = \frac{1}{x^3}$ which is monotone decreasing on $x<0$ and $x>0$ separately, however it is not monotone on the full real-line.
...
\endgroup
\begin{proof}
	The bound is a direct consequence of the properties of the $\alpha$--R\'{e}nyi diveregence, proven in Theorem~\ref{thm:Da_props}.
