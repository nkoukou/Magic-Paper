...
The vector of ratios $\widetilde{\bmw}(\rho|\sigma)$ used to construct the curve and defined as
\begin{equation}
...
Without loss of generality, we can always write a stabiler state $\sigma$ as a Gibbs state $\sigma = \frac{1}{\Z_\beta} e^{-\beta H}$ for some $\beta \geq 0$ and Hamiltonian $H$ as discussed in~\cref{sec:major_frag}.
...
\nick{Specify: How general can the states get? What about not full-rank states? Should we include the case of odd $n$ or non-zero $\epsilon'$ in the appendix? What about $\epsilon>0$?}
